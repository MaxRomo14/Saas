...
:root {
  --bg:#eef0f3; --card:#1e2128; --card2:#272b34; --border:#333844;
  --navy:#13161c; --blue:#3b82f6; --blue2:#2563eb;
  --tw:#f1f3f6; --tw2:#9aa0b0; --lw:#111827; --lw2:#4b5563;
  --red:#ef4444; --green:#22c55e; --yellow:#f59e0b;
  --sans:'Inter',-apple-system,sans-serif;
}
html,body,[class*="css"]{font-family:var(--sans)!important;font-size:14px!important;-webkit-font-smoothing:antialiased!important;}
/* BG */
.stApp,[data-testid="stAppViewContainer"],[data-testid="stAppViewBlockContainer"],
section.main,.main,.block-container{background:#eef0f3!important;}
.main .block-container{padding-top:1.8rem!important;max-width:1200px!important;}
/* Sidebar */
section[data-testid="stSidebar"],section[data-testid="stSidebar"]>div{background:var(--navy)!important;}
section[data-testid="stSidebar"] *{color:#c8d0e0!important;}
section[data-testid="stSidebar"] .stRadio label{font-size:13px!important;font-weight:500!important;}
/* Inputs */
.stTextInput>div>div>input,.stTextArea>div>div>textarea,.stSelectbox>div>div,.stDateInput>div>div>input{
  background:#272b34!important;border:1px solid #333844!important;color:#f1f3f6!important;
  border-radius:6px!important;font-size:14px!important;}
.stTextInput>div>div>input:focus,.stTextArea>div>div>textarea:focus{border-color:var(--blue)!important;}
/* all labels on grey bg */
label{font-size:11px!important;font-weight:600!important;color:#374151!important;text-transform:uppercase!important;letter-spacing:.05em!important;}
/* Buttons */
.stButton>button{background:var(--blue)!important;color:#fff!important;border:none!important;
  font-weight:600!important;font-size:13px!important;border-radius:6px!important;padding:.45rem 1.2rem!important;
  box-shadow:0 1px 3px rgba(59,130,246,.3)!important;transition:all .15s!important;}
.stButton>button:hover{background:var(--blue2)!important;transform:translateY(-1px)!important;}
/* Tabs */
.stTabs [data-baseweb="tab-list"]{background:var(--card)!important;border-bottom:1px solid var(--border)!important;border-radius:8px 8px 0 0!important;}
.stTabs [data-baseweb="tab"]{background:transparent!important;color:var(--tw2)!important;font-size:13px!important;
  font-weight:500!important;padding:.7rem 1.4rem!important;border-bottom:2px solid transparent!important;margin-bottom:-1px!important;}
...
[data-testid="stMetric"]{background:var(--card)!important;border:1px solid var(--border)!important;
  border-radius:10px!important;padding:1.2rem!important;box-shadow:0 2px 8px rgba(0,0,0,.25)!important;}
[data-testid="stMetricLabel"]{color:var(--tw2)!important;font-size:11px!important;font-weight:600!important;text-transform:uppercase!important;letter-spacing:.06em!important;}
[data-testid="stMetricValue"]{color:var(--tw)!important;font-size:1.9rem!important;font-weight:700!important;}
[data-testid="stMetricDelta"]{color:var(--tw2)!important;}
/* Expanders */
.streamlit-expanderHeader{background:var(--card)!important;border:1px solid var(--border)!important;
  border-radius:8px!important;font-size:14px!important;font-weight:600!important;box-shadow:0 1px 4px rgba(0,0,0,.2)!important;}
.streamlit-expanderHeader,.streamlit-expanderHeader *{color:var(--tw)!important;}
.streamlit-expanderContent{background:var(--card2)!important;border:1px solid var(--border)!important;
  border-top:none!important;border-radius:0 0 8px 8px!important;}
.streamlit-expanderContent,.streamlit-expanderContent *{color:var(--tw)!important;}
/* Dataframe */
.stDataFrame thead th{background:var(--card2)!important;color:var(--tw2)!important;font-size:11px!important;font-weight:600!important;text-transform:uppercase!important;}
.stDataFrame tbody td{color:var(--tw)!important;font-size:13px!important;}
/* Cards */
.gc-card{background:var(--card);border:1px solid var(--border);border-radius:10px;padding:1.2rem 1.4rem;margin-bottom:.75rem;box-shadow:0 2px 8px rgba(0,0,0,.2);}
.gc-card,.gc-card *{color:var(--tw)!important;}
.gc-card-header{font-size:11px;font-weight:700;letter-spacing:.08em;color:var(--tw2)!important;text-transform:uppercase;margin-bottom:.6rem;}
/* Badges */
.gc-badge{display:inline-block;padding:.18rem .6rem;border-radius:20px;font-size:11px;font-weight:600;}
.badge-open{background:rgba(59,130,246,.25);color:#93c5fd!important;}
.badge-closed{background:rgba(34,197,94,.25);color:#86efac!important;}
.badge-overdue{background:rgba(239,68,68,.25);color:#fca5a5!important;}
.badge-pending{background:rgba(251,191,36,.25);color:#fde68a!important;}
.badge-high{background:rgba(239,68,68,.25);color:#fca5a5!important;}
.badge-med{background:rgba(251,191,36,.25);color:#fde68a!important;}
.badge-low{background:rgba(34,197,94,.25);color:#86efac!important;}
.badge-draft{background:rgba(156,163,175,.25);color:#d1d5db!important;}
/* Timeline */
.gc-tl{border-left:2px solid #404656;padding:.3rem 0 .3rem 1rem;margin-bottom:.4rem;position:relative;font-size:13px;color:var(--tw2)!important;line-height:1.5;}
.gc-tl *{color:var(--tw2)!important;}
.gc-tl::before{content:'';position:absolute;left:-5px;top:.55rem;width:8px;height:8px;border-radius:50%;background:var(--blue);}
/* Header */
.gc-header{background:var(--card);border:1px solid var(--border);border-left:4px solid var(--blue);border-radius:10px;padding:1.2rem 1.6rem;margin-bottom:1.5rem;box-shadow:0 2px 8px rgba(0,0,0,.2);}
.gc-header h1{font-size:1.2rem;font-weight:700;color:var(--tw)!important;margin:0 0 .2rem 0;letter-spacing:-.01em;}
.gc-header p{font-size:13px;color:var(--tw2)!important;margin:0;}
/* Alerts */
.gc-alert{border-radius:6px;padding:.65rem 1rem;margin:.5rem 0;font-size:13px;line-height:1.5;}
.gc-alert,.gc-alert *{color:inherit!important;}
.gc-alert-warn{background:rgba(245,158,11,.15);border-left:3px solid var(--yellow);color:#fcd34d!important;}
.gc-alert-danger{background:rgba(239,68,68,.15);border-left:3px solid var(--red);color:#fca5a5!important;}
.gc-alert-ok{background:rgba(34,197,94,.15);border-left:3px solid var(--green);color:#86efac!important;}
/* Section dividers & misc */
hr{border:none;border-top:1px solid var(--border)!important;margin:1.2rem 0!important;}
p,li{color:var(--tw)!important;line-height:1.6!important;}
b,strong{color:var(--tw)!important;}
code{font-size:12px!important;background:rgba(59,130,246,.15)!important;color:#93c5fd!important;padding:.1rem .3rem!important;border-radius:4px!important;}
::-webkit-scrollbar{width:5px;height:5px;}
::-webkit-scrollbar-track{background:var(--navy);}
::-webkit-scrollbar-thumb{background:#404656;border-radius:3px;}
/* Checklist section highlight */
.cl-owner-card{background:var(--card);border:1px solid var(--border);border-top:3px solid var(--blue);border-radius:10px;padding:1rem 1.3rem;margin-bottom:1rem;box-shadow:0 2px 8px rgba(0,0,0,.2);}
.cl-owner-card *{color:var(--tw)!important;}
.cl-task-row{background:var(--card2);border:1px solid var(--border);border-radius:6px;padding:.7rem 1rem;margin:.4rem 0;font-size:13px;}
.cl-task-row *{color:var(--tw)!important;}
/* Login */
.login-wrap{max-width:400px;margin:6vh auto;}
.login-logo{font-size:1.8rem;font-weight:700;color:var(--lw)!important;letter-spacing:-.02em;text-align:center;margin-bottom:.2rem;}
.login-sub{text-align:center;color:var(--lw2)!important;font-size:12px;margin-bottom:2rem;text-transform:uppercase;letter-spacing:.1em;font-weight:500;}
</style>
...
def sidebar():
    with st.sidebar:
        ...
        <div style='font-size:.95rem;font-weight:700;color:#f1f3f6;margin-bottom:.1rem'>{user['name']}</div>
        <div style='font-size:12px;color:#60a5fa;margin-bottom:.8rem'>{user.get('title', user['role'])}</div>
        ...
        pages = [
            ("🏠","Dashboard"),("📋","Tasks & Remediation"),("📁","Exam Requests"),
            ("📚","WSP Repository"),(f"✅","Supervisory Checklists"),
            ("🔍","Evidence Repository"),("📏","Rule Inventory"),("🤖","AI Rule Scan"),
            ("🏢","Org Chart"),("🔒","Audit Trail"),("⚙️","Firm Config"),("💾","Data Export"),
        ]
        # Show checklist count badge
        labels = []
        for icon, name in pages:
            if name == "Supervisory Checklists" and cl_count > 0:
                labels.append(f"{icon}  {name} ({cl_count})")
            else:
                labels.append(f"{icon}  {name}")

        nav_labels = [p[1] for p in pages]
        nav = st.radio("nav", nav_labels, format_func=lambda x: labels[nav_labels.index(x)], label_visibility="collapsed")
...
def page_dashboard():
    ...
    with cl:
        st.markdown("<div style='font-size:11px;font-weight:700;letter-spacing:.08em;color:#9aa0b0;text-transform:uppercase;margin-bottom:.8rem'>MY TASKS</div>", unsafe_allow_html=True)
        ...
        for t in my_tasks[:6]:
            ...
            st.markdown(f"""<div class='gc-card' style='padding:.8rem 1rem;margin-bottom:.4rem'>
              <div style='display:flex;justify-content:space-between;align-items:center'>
                <div><span style='font-size:11px;color:#9aa0b0'>{t['id']}</span>
                <span style='font-weight:600;margin-left:.5rem'>{t['title']}</span></div>
                <div>{badge(s,sc)}&nbsp;{badge(p,pc)}</div>
              </div>
              <div style='margin-top:.3rem;font-size:12px;color:#9aa0b0'>Due: {t['due']} · {uname} · {t['category']}</div>
            </div>""", unsafe_allow_html=True)
    with cr:
        st.markdown("<div style='font-size:11px;font-weight:700;letter-spacing:.08em;color:#9aa0b0;text-transform:uppercase;margin-bottom:.8rem'>RECENT ACTIVITY</div>", unsafe_allow_html=True)
        for e in reversed(D["audit_trail"][-8:]):
            st.markdown(f"<div class='gc-tl'><span style='color:#60a5fa;font-size:11px'>{e['ts']}</span> · <b>{e['user']}</b><br/>{e['action']}</div>", unsafe_allow_html=True)
    ...
    with tab1:
        ...
        for t in tasks:
            ...
            with st.expander(f"[{t['id']}]  {t['title']}"):
                ...
                with st.expander("🚪 Close with Rationale"):
                ...
                for e in t["audit"]:
                    st.markdown(f"<div class='gc-tl'><span style='color:#60a5fa;font-size:11px'>{e['ts']}</span> · <b>{e['user']}</b> · {e['action']}</div>", unsafe_allow_html=True)
    with tab2:
        c1,c2=st.columns(2)
    ...
    with tab1:
        for wsp in D["wsps"]:
            ...
            with st.expander(f"[{wsp['id']}]  {wsp['title']}  ·  v{wsp['version']}"):
                ...
                st.markdown(f"<div style='font-size:13px;color:#9aa0b0;line-height:1.7;padding:.5rem 0'>{wsp['content']}</div>", unsafe_allow_html=True)
                ...
                if not wsp_task_list:
                    ...
                    st.markdown(f"<div style='font-size:11px;font-weight:700;letter-spacing:.08em;color:#9aa0b0;text-transform:uppercase;margin-bottom:.8rem'>📋 SUPERVISORY CHECKLIST GENERATOR</div>", unsafe_allow_html=True)
                    # group by owner
                    owners = {}
                    ...
                    for owner_key, tasks_for_owner in owners.items():
                        ...
                        <div class='cl-owner-card'>
                          <div style='font-size:12px;color:#60a5fa;font-weight:700;text-transform:uppercase;letter-spacing:.06em;margin-bottom:.3rem'>
                            {oinfo.get('title', oinfo.get('role',''))} — {oinfo.get('name',owner_key)}
                          </div>
                          <div style='font-size:12px;color:#9aa0b0;margin-bottom:.7rem'>{len(tasks_for_owner)} obligation(s) identified in {wsp['id']}</div>
                        """, unsafe_allow_html=True)
                        for wt in tasks_for_owner:
                            ...
                            <div class='cl-task-row'>
                              ...
                              <div style='font-size:12px;color:#9aa0b0;margin-bottom:.2rem'>🔁 {wt['frequency']}</div>
                              <div style='font-size:12px;color:#9aa0b0'>{wt['description']}</div>
                            </div>
                            """, unsafe_allow_html=True)
                        st.markdown("</div>", unsafe_allow_html=True)
...
def page_checklists():
    ...
    for cl in checklists:
        ...
        with st.expander(f"[{cl['id']}]  {cl['owner_name']} — {cl['wsp_title']}  ·  {done}/{total} complete"):
            # Header info
            st.markdown(f"""
            <div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:.8rem'>
              <div>
                <div style='font-size:13px;color:#60a5fa;font-weight:700'>{cl['owner_title']}</div>
                <div style='font-size:1rem;font-weight:700;margin:.1rem 0'>{cl['owner_name']}</div>
                <div style='font-size:12px;color:#9aa0b0'>Source: {cl['source_wsp']} · Due: {cl['due']} · {cl['recurrence']}</div>
              </div>
            ...
            for idx,item in enumerate(cl["items"]):
                ...
                <div class='cl-task-row' style='border-left:3px solid {"#22c55e" if item["checked_off"] else "#3b82f6"}'>
                  <div style='display:flex;justify-content:space-between;align-items:flex-start'>
                    <div style='flex:1'>
                      ...
                      <div style='font-size:12px;color:#9aa0b0;margin-bottom:.3rem'>🔁 {item['frequency']} &nbsp;·&nbsp; {badge(item['category'],cat_cls)}</div>
                      <div style='font-size:13px;color:#c8d0e0;line-height:1.6'>{item['description']}</div>
                    </div>
                  </div>
                """, unsafe_allow_html=True)
                ...
                if is_mine and not item["checked_off"]:
                    ...
                    st.markdown(f"<div style='font-size:12px;color:#86efac;margin:.3rem 0 .6rem 0'>✅ Completed {item['completed_ts']} by {item['audit'][-1]['user'] if item['audit'] else '–'}</div>", unsafe_allow_html=True)

                # Item audit trail
                with st.expander(f"View audit trail for: {item['title'][:50]}",expanded=False):
                    for e in item["audit"]:
                        st.markdown(f"<div class='gc-tl'><span style='color:#60a5fa;font-size:11px'>{e['ts']}</span> · <b>{e['user']}</b> · {e['action']}</div>", unsafe_allow_html=True)
                st.markdown("<hr/>", unsafe_allow_html=True)

            # Checklist-level audit trail
            with st.expander("📋 Full Checklist Audit Trail"):
                for e in cl["audit"]:
                    st.markdown(f"<div class='gc-tl'><span style='color:#60a5fa;font-size:11px'>{e['ts']}</span> · <b>{e['user']}</b> · {e['action']}</div>", unsafe_allow_html=True)
...
def page_evidence():
    ...
    for ev in ev_list:
        ...
        st.markdown(f"""<div class='gc-card'>
          ...
          <div style='margin-top:.3rem;font-size:12px;color:#9aa0b0'>
    ...
    with tab1:
        for r in D["rules"]:
            ...
            with st.expander(f"[{r['id']}]  {r['title']}{flag}"):
                ...
                st.markdown(f"<div style='font-size:13px;color:#9aa0b0;margin:.5rem 0'>{r['description']}</div>", unsafe_allow_html=True)
    ...
    if D["rule_scan_memo"]:
        ...
        for ch in m["changes"]:
            ...
            st.markdown(f"""<div class='gc-card'>
              ...
              <div style='font-size:13px;color:#9aa0b0;margin:.4rem 0'>{ch['change']}</div>
...
def page_org():
    ...
    for node in D["org_chart"]:
        ...
        st.markdown(f"""<div class='gc-card'>
          <div style='font-size:12px;color:#60a5fa;font-weight:700;letter-spacing:.05em'>{node['role']}</div>
          <div style='font-size:1rem;font-weight:700;margin:.2rem 0'>{u.get('name','–')} <span style='color:#9aa0b0;font-size:.8rem'>@{node['user']}</span></div>
          <div style='font-size:12px;color:#9aa0b0;margin-bottom:.5rem'>{rep}</div>
...
def page_audit():
    ...
    st.markdown(f"<div style='font-size:12px;color:#9aa0b0;margin-bottom:.8rem'>{len(trail)} records</div>", unsafe_allow_html=True)
    for e in trail:
        st.markdown(f"<div class='gc-tl'><span style='color:#60a5fa;font-size:11px'>{e['ts']}</span> &nbsp;{badge(e['module'],'badge-open')}&nbsp; <b>{e['user']}</b> · {e['action']}</div>", unsafe_allow_html=True)
