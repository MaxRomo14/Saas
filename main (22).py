    ...
    with tab1:
        for er in D["exam_requests"]:
            ...
            if toggle(f"er_{er['id']}", f"{er['id']} — {er['title']} | {er['regulator']} | Due: {er['due']}"):
                ...
                st.markdown(
                    f"<div style='background:#f0f7ff;border:1px solid #bfdbfe;border-radius:8px;"
                    f"padding:.7rem 1rem;margin-bottom:.8rem;display:flex;align-items:center;"
                    f"justify-content:space-between;gap:.5rem'>"
                    f"<span style='font-size:13px;font-weight:600;color:#1e40af'>📄 Original Request List — {er['title']}</span>"
                    f"<span style='font-size:12px;color:#6b7280'>{len(er['items'])} item(s) · click below to download</span>"
                    f"</div>",
                    unsafe_allow_html=True
                )
                st.download_button(
                    ...
                    data=request_txt,
                    file_name=f"{er['id']}_request_list.txt",
                    mime="text/plain",
                    key=f"dl_req_{er['id']}",
                )
...
def page_evidence():
    ...
    for ev in ev_list:
        arch_badge   = f" {badge('ARCHIVED','badge-pending')}" if ev["archived"] else ""
        notes_line   = f"<div style='margin-top:.3rem;font-size:12px;color:#374151'>💬 <i>{ev['notes']}</i></div>" if ev.get("notes") else ""
        uploader     = D["users"].get(ev["uploaded_by"],{}).get("name", ev["uploaded_by"])
        st.markdown(f"""<div class='gc-card'>
          <div style='display:flex;justify-content:space-between;align-items:center'>
            <div><span style='font-size:11px;color:#60a5fa'>{ev['id']}</span>
            <span style='font-weight:600;margin-left:.5rem'>📎 {ev['filename']}</span>{arch_badge}</div>
            {badge('IMMUTABLE · WORM','badge-closed')}
          </div>
          <div style='margin-top:.3rem;font-size:12px;color:#6b7280'>
            {uploader} · {ev['ts']} · Task: {ev['task_id']} · <code>{ev['hash']}</code>
          </div>
          {notes_line}
        </div>""", unsafe_allow_html=True)
    st.markdown("<hr/>", unsafe_allow_html=True)
