...
def page_wsp():
    ...
    # ── Shared: render one approved doc row + detail expand ───────────────────
    def _render_library(docs, tasks_key, task_dict_key, id_prefix, lob_filter, search_filter):
        approved = [w for w in docs if w["status"] == "Approved"]
        if lob_filter    != "All": approved = [w for w in approved if w.get("line_of_business") == lob_filter]
        if search_filter:
            q = search_filter.lower()
            approved = [w for w in approved if q in w["title"].lower() or q in w["id"].lower()]

        if not approved:
            st.markdown("<div class='gc-alert gc-alert-warn'>No approved documents match the current filters.</div>", unsafe_allow_html=True)
            return

        from collections import defaultdict as _ddf
        by_lob = _ddf(list)
        for w in approved:
            by_lob[w.get("line_of_business","Operations")].append(w)

        for lob in sorted(by_lob.keys()):
            lob_docs = by_lob[lob]
            st.markdown(f"""
            <div style='background:#1e293b;border-radius:8px;
                 padding:.6rem 1rem;margin:1rem 0 .3rem 0;
                 display:flex;justify-content:space-between;align-items:center'>
              <span style='font-size:14px;font-weight:800;letter-spacing:.05em;color:#f8fafc !important;-webkit-text-fill-color:#f8fafc'>📁 {lob.upper()}</span>
              <span style='font-size:12px;color:#94a3b8 !important;-webkit-text-fill-color:#94a3b8'>{len(lob_docs)} document(s)</span>
            </div>
            """, unsafe_allow_html=True)
            ...
            for doc in lob_docs:
                ...
                # Pre-build conditional snippets — never use ternary f-strings inside markdown
                task_snippet = f"&nbsp;·&nbsp; <span style='color:#2563eb'>{task_count} procedure(s)</span>" if task_count else ""
                cl_snippet   = f"&nbsp;·&nbsp; <span style='color:#7c3aed'>{cl_count} checklist(s)</span>" if cl_count else ""

                # Download doc
                doc_lines = [
                    f"{'='*60}",
                    f"  {doc['id']} — {doc['title']}",
                    f"  Type: {doc.get('type','Document')}  |  Line of Business: {doc.get('line_of_business','')}",
                    f"  Version: {doc['version']}  |  Status: {doc['status']}",
                    f"  Owner: {owner_name}",
                    f"  Approved By: {approver_name}  |  Date: {doc['approved_ts'][:10]}",
                    f"{'='*60}", "",
                    "CONTENT", "-"*60,
                    doc.get("content",""), "",
                    "VERSION HISTORY", "-"*60,
                ...
                doc_b64  = _b64w.b64encode("\n".join(doc_lines).encode()).decode()
                doc_fname = f"{doc['id']}_v{doc['version']}.txt"

                st.markdown(f"""
                <div style='background:#ffffff;border:1px solid #e5e7eb;border-left:4px solid #22c55e;
                     border-radius:8px;padding:.7rem 1.1rem;margin-bottom:4px;
                     display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.5rem'>
                  <div style='min-width:0;flex:1'>
                    <div style='display:flex;align-items:center;gap:.5rem;flex-wrap:wrap'>
                      <span style='font-size:11px;font-weight:800;color:#22c55e;white-space:nowrap'>{doc['id']}</span>
                      <span style='font-size:13px;font-weight:700;color:#111111'>📄 {doc['title']}</span>
                      <span style='background:#dcfce7;color:#166534;font-size:10px;font-weight:700;padding:1px 8px;border-radius:10px;white-space:nowrap'>v{doc['version']}</span>
                    </div>
                    <div style='font-size:11px;color:#6b7280;margin-top:3px'>
                      Owner: <b>{owner_name}</b> &nbsp;·&nbsp; Approved by: <b>{approver_name}</b> &nbsp;·&nbsp; {doc['approved_ts'][:10]}{task_snippet}{cl_snippet}
                    </div>
                  </div>
                  <a href="data:text/plain;base64,{doc_b64}" download="{doc_fname}"
                     style='background:#2563eb;color:#ffffff !important;font-size:11px;font-weight:700;
                ...
                if toggle(f"doc_det_{id_prefix}_{doc['id']}", f"📋  Details & Procedures — {doc['id']}"):
                    # Overview
                    st.markdown(f"""
                    <div style='background:#f0fdf4;border:1px solid #bbf7d0;border-left:4px solid #22c55e;
                         border-radius:8px;padding:.8rem 1.1rem;margin:.4rem 0 .6rem 0;display:flex;gap:2.5rem;flex-wrap:wrap'>
                      <div><div style='font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase'>Line of Business</div>
                        <div style='font-size:13px;font-weight:600;color:#111'>{doc.get('line_of_business','—')}</div></div>
                      ...
                      <div><div style='font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase'>Approved</div>
                        <div style='font-size:13px;font-weight:600;color:#111'>{doc['approved_ts'][:10]}</div></div>
                    ...
                    st.markdown("<div style='font-size:11px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:.3rem'>VERSION HISTORY</div>", unsafe_allow_html=True)
                    for v in doc.get("versions",[]):
                        st.markdown(f"<div class='gc-tl'>v{v['ver']} · {v['ts']} · {v['author']} · <i>{v['note']}</i></div>", unsafe_allow_html=True)

                    st.markdown("<hr/><div style='font-size:11px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin:.4rem 0'>SUPERVISORY PROCEDURES</div>", unsafe_allow_html=True)
                    if not task_list:
                        ...
                        for owner_key, owner_tasks in by_owner.items():
                            ...
                            st.markdown(f"""<div class='cl-owner-card'>
                              ...
                              <div style='font-size:12px;color:#6b7280;margin-bottom:.6rem'>{len(owner_tasks)} procedure(s) assigned</div>""", unsafe_allow_html=True)
                    ...
                    if existing_cls:
                        st.markdown(f"<div class='gc-alert gc-alert-ok'>✓ {len(existing_cls)} active checklist(s) from this document (v{doc['version']}). Checklists auto-update on new version approval.</div>", unsafe_allow_html=True)
                        if st.button(f"🔄 Re-Parse & Regenerate Checklists — {doc['id']}", key=f"regen_{id_prefix}_{doc['id']}"):
                        ...
                        if st.button(f"⚙️ Parse & Generate Checklists from {doc['id']}", key=f"gen_cl_{id_prefix}_{doc['id']}"):
                            n = _auto_parse_wsp(doc)
                            add_audit("Policies & WSPs", f"{doc['id']}: parsed, {n} checklist(s) generated")
    ...
    with tab_pol:
        lobs_pol = sorted(set(w.get("line_of_business","Operations") for w in D.get("policies",[]) if w["status"]=="Approved"))
        fc1, fc2 = st.columns(2)
        f_lob_pol    = fc1.selectbox("Filter by Line of Business", ["All"] + lobs_pol, key="pol_lob_filter")
        f_search_pol = fc2.text_input("🔍 Search", placeholder="title, ID…", key="pol_search")
        _render_library(D.get("policies",[]), "policy_tasks", "policy_tasks", "pol", f_lob_pol, f_search_pol)

        if toggle("new_pol_toggle", "➕ New Policy"):
            _render_new_form("Policy", "policies", "policy_tasks", "POL")

    with tab_wsp:
        lobs_wsp = sorted(set(w.get("line_of_business","Operations") for w in D["wsps"] if w["status"]=="Approved"))
        fc3, fc4 = st.columns(2)
        f_lob_wsp    = fc3.selectbox("Filter by Line of Business", ["All"] + lobs_wsp, key="wsp_lob_filter")
        f_search_wsp = fc4.text_input("🔍 Search", placeholder="title, ID…", key="wsp_search")
        _render_library(D["wsps"], "wsp_tasks", "wsp_tasks", "wsp", f_lob_wsp, f_search_wsp)
