...
def seed_data():
    ...
    policy_tasks = {
        ...
        "POL-002": [
            ...
            {"id":"PT-002-2","title":"Quarterly Supervisory Controls Report","owner":"supervisor","role_title":"Supervising Principal","frequency":"Quarterly","description":"Compile the quarterly supervisory controls report summarizing review activity, exceptions identified, and remediation status across all business lines.","category":"Supervisory"},
        ],
        "POL-003": [
    ...
    return {
        ...
        "policies": [
            ...
            },
            {
                ...
                "status":"Approved","review_status":"approved","line_of_business":"Wealth Management",
                "type":"Policy",
                "owner":"supervisor","approved_by":"admin","approved_ts":ts(35),
        ...
        "compliance_tests": [
            {
                ...
                "procedures":[
                    {"id":"P-001","step":1,
                     ...
                     "description":"Examiner requested evidence of the Quarterly Business Continuity Plan Review for each quarter in 2025.",
                     "notes":"The designated supervisor provided all requested evidence of the quarterly reviews for 2025, and evidence of supervisor approval.",
                    ...
                    {"id":"P-002","step":2,
                     ...
                     "description":"Examiner requested evidence that supervisory sign-off was documented for each flagged trade in the sample period.",
                     "notes":"Sign-off documentation present in all sampled files. Format consistent with WSP-002 requirements.",
                     "result":"Pass","exception":False,"has_remediation_item":False,
                     "tested_by":"supervisor","tested_ts":ts(4)},
                    {"id":"P-003","step":3,
                     "title":"CCO Escalation Review — SLA Compliance",
                     "description":"Examiner requested evidence that escalated trades received CCO review within the required 48-hour timeframe.",
                     "notes":"3 of 4 escalated trades received CCO review within timeframe. 1 trade (account #4471) escalated on 2/14 but CCO review not documented until 2/19 — 5 days over SLA.",
                     "result":"Exception","exception":True,"has_remediation_item":True,
                     "rem_name":"Update CCO escalation SLA monitoring procedure",
                     "rem_detail":"Implement daily automated alert for escalated trades approaching 48-hour CCO review deadline. Update WSP-002 to require supervisor to flag any CCO review delays same-day.",
...
def _auto_parse_wsp(wsp):
    """Called on WSP approval. Reads wsp_tasks for this WSP and upserts supervisory checklists.
    If checklists already exist for this WSP, removes old ones and regenerates so they always
    reflect the current approved version."""
    now_str   = datetime.now().strftime("%Y-%m-%d %H:%M")
    wsp_id    = wsp["id"]
    task_list = D["wsp_tasks"].get(wsp_id, [])
    if not task_list:
        return 0

    # Remove any existing checklists for this WSP (version update)
    D["supervisory_checklists"] = [c for c in D.get("supervisory_checklists",[]) if c.get("source_wsp") != wsp_id]
    ...
    for owner_key, tasks in owners.items():
        ...
        for wt in tasks:
            cl_items.append({
                ...
                "supervisor_notes":"",
                "audit":[{"ts":now_str,"user":"system","action":f"Task generated from {wsp_id} v{wsp['version']} on approval"}],
            })
        D["supervisory_checklists"].append({
            "id":cl_id,"source_wsp":wsp_id,"wsp_title":wsp["title"],
            "wsp_version":wsp["version"],"line_of_business":wsp.get("line_of_business",""),
            ...
            "audit":[{"ts":now_str,"user":"system","action":f"Auto-generated from {wsp_id} v{wsp['version']} approval"}],
        })
    return len(owners)


def page_wsp():
    st.markdown("<div class='gc-header'><h1>POLICIES & WSPs</h1><p>Policy and procedure library organized by line of business — folder view, version control, and automatic checklist generation on approval</p></div>", unsafe_allow_html=True)
    ...
    def _render_library(docs, tasks_key, id_prefix):
        ...
        f_search  = tb1.text_input("🔍 Search", placeholder="name, ID, owner…", key=f"{id_prefix}_search")
        ...
        f_lob     = tb2.selectbox("Line of Business", ["All"] + all_lobs,  key=f"{id_prefix}_lob")
        f_owner   = tb3.selectbox("Owner",            ["All"] + all_owners, key=f"{id_prefix}_owner")
        f_sort    = tb4.selectbox("Sort by", ["Name ↑","Name ↓","Date ↑","Date ↓","Version ↑","Version ↓"], key=f"{id_prefix}_sort")
        f_view    = tb5.selectbox("View",    ["Grouped by LOB","Flat list"], key=f"{id_prefix}_view")

        # Ctrl row: expand/collapse all + doc count
        cc1, cc2, cc3 = st.columns([1.2, 1.2, 5])
        if cc1.button("⊞ Expand All",  key=f"{id_prefix}_expall"):
            for w in approved: st.session_state[f"docrow_{id_prefix}_{w['id']}"] = True
        if cc2.button("⊟ Collapse All", key=f"{id_prefix}_colall"):
        ...
        if f_lob   != "All": filtered = [w for w in filtered if w.get("line_of_business") == f_lob]
        if f_owner != "All": filtered = [w for w in filtered if D["users"].get(w["owner"],{}).get("name",w["owner"]) == f_owner]

        # Apply sort
        def _sort_key(w):
            if   "Name"    in f_sort: return w["title"].lower()
            elif "Date"    in f_sort: return w.get("approved_ts","")
            elif "Version" in f_sort: return w.get("version","")
            return w["title"].lower()
        rev = "↓" in f_sort
        filtered = sorted(filtered, key=_sort_key, reverse=rev)

        cc3.markdown(f"<div style='font-size:12px;color:#6b7280;padding:.45rem 0'>{len(filtered)} document(s) shown</div>", unsafe_allow_html=True)
        ...
        # ── Table header ──────────────────────────────────────────────────────
        st.markdown("""
        <div style='display:grid;grid-template-columns:90px 1fr 140px 120px 120px 90px 70px 195px;
             gap:0;background:#f1f5f9;border:1px solid #e2e8f0;border-radius:6px 6px 0 0;
             padding:.4rem .8rem;margin-top:.6rem'>
          <div style='font-size:10px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.07em'>ID</div>
          <div style='font-size:10px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.07em'>Document Name</div>
          <div style='font-size:10px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.07em'>Line of Business</div>
          <div style='font-size:10px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.07em'>Owner</div>
          <div style='font-size:10px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.07em'>Approved By</div>
          <div style='font-size:10px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.07em'>Date</div>
          <div style='font-size:10px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.07em'>Ver</div>
          <div style='font-size:10px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.07em'>Actions</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        for grp_label, grp_docs in groups:
            if grp_label:
                st.markdown(f"""
                <div style='background:#1e293b;padding:.38rem .8rem;
                     border-left:1px solid #334155;border-right:1px solid #334155;'>
                  <span style='font-size:11px;font-weight:800;letter-spacing:.08em;
                               color:#f8fafc !important;-webkit-text-fill-color:#f8fafc'>
                    📁 {grp_label.upper()} &nbsp;<span style='font-weight:400;color:#94a3b8 !important;-webkit-text-fill-color:#94a3b8'>{len(grp_docs)} doc(s)</span>
                  </span>
                </div>
                """, unsafe_allow_html=True)
            ...
            for i, doc in enumerate(grp_docs):
                ...
                doc_lines = [
                    f"{'='*60}",
                    f"  {doc['id']} — {doc['title']}",
                    f"  Type: {doc.get('type','Document')}  |  LOB: {doc.get('line_of_business','')}",
                    f"  Version: {doc['version']}  |  Status: {doc['status']}",
                    f"  Owner: {owner_name}  |  Approved By: {approver_name}  |  Date: {doc.get('approved_ts','')[:10]}",
                ...
                st.markdown(f"""
                <div style='display:grid;grid-template-columns:90px 1fr 140px 120px 120px 90px 70px 195px;
                     gap:0;background:{row_bg};border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;
                     border-bottom:1px solid #e2e8f0;padding:.45rem .8rem;align-items:center;
                     {"border-left:3px solid #3b82f6;" if is_expanded else ""}'>
                  <div style='font-size:11px;font-weight:800;color:#059669;white-space:nowrap'>{doc['id']}</div>
                  <div style='font-size:12px;font-weight:600;color:#111827;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'>📄 {doc['title']}</div>
                  <div style='font-size:11px;color:#4b5563;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'>{lob_short}</div>
                  <div style='font-size:11px;color:#4b5563;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'>{owner_name}</div>
                  <div style='font-size:11px;color:#4b5563;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'>{approver_name}</div>
                  <div style='font-size:11px;color:#4b5563;white-space:nowrap'>{approved_date}</div>
                  <div><span style='background:#dcfce7;color:#166534;font-size:10px;font-weight:700;padding:1px 7px;border-radius:10px'>v{doc['version']}</span></div>
                  <div style='display:flex;gap:5px;align-items:center'>
                    <a href="data:text/plain;base64,{doc_b64}" download="{doc_fname}"
                       style='background:#2563eb;color:#fff !important;font-size:10px;font-weight:700;
                              padding:3px 9px;border-radius:4px;text-decoration:none;white-space:nowrap'>⬇ Download</a>
                  </div>
                </div>
                """, unsafe_allow_html=True)

                # Expand Details button sits in a separate Streamlit column row aligned under Actions
                btn_cols = st.columns([90/930, 1/930*470, 140/930, 120/930, 120/930, 90/930, 70/930, 195/930])
                with btn_cols[7]:
                    if st.button(f"{exp_icon} Details", key=f"expbtn_{id_prefix}_{doc['id']}"):
                ...
                if is_expanded:
                    ...
                    cl_count  = len([c for c in D.get("supervisory_checklists",[]) if c.get("source_wsp")==doc["id"]])

                    with st.container():
                        st.markdown(f"""
                        <div style='background:#f8fafc;border-left:3px solid #3b82f6;
                             border:1px solid #bfdbfe;border-left:3px solid #3b82f6;
                             border-radius:0 0 6px 6px;padding:1rem 1.2rem;margin-bottom:2px'>
                          <div style='display:flex;gap:2.5rem;flex-wrap:wrap;margin-bottom:.8rem;
                               padding-bottom:.8rem;border-bottom:1px solid #e2e8f0'>
                            <div><div style='font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase;margin-bottom:.2rem'>Line of Business</div>
                              <div style='font-size:13px;font-weight:600;color:#111'>{lob_short or "—"}</div></div>
                            <div><div style='font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase;margin-bottom:.2rem'>Owner</div>
                              <div style='font-size:13px;font-weight:600;color:#111'>{owner_name}</div></div>
                            <div><div style='font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase;margin-bottom:.2rem'>Approved By</div>
                              <div style='font-size:13px;font-weight:600;color:#111'>{approver_name}</div></div>
                            <div><div style='font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase;margin-bottom:.2rem'>Approval Date</div>
                              <div style='font-size:13px;font-weight:600;color:#111'>{approved_date}</div></div>
                            <div><div style='font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase;margin-bottom:.2rem'>Version</div>
                              <div style='font-size:13px;font-weight:600;color:#111'>v{doc['version']}</div></div>
                          </div>
                        </div>
                        """, unsafe_allow_html=True)

                        # Version history
                        st.markdown("<div style='font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.07em;margin:.6rem 0 .3rem 0'>VERSION HISTORY</div>", unsafe_allow_html=True)
                        for v in doc.get("versions",[]):
                            st.markdown(f"<div class='gc-tl'>v{v['ver']} · {v['ts']} · {v['author']} · <i>{v['note']}</i></div>", unsafe_allow_html=True)

                        # Supervisory procedures
                        st.markdown("<div style='font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.07em;margin:.8rem 0 .3rem 0'>SUPERVISORY PROCEDURES</div>", unsafe_allow_html=True)
                        if not task_list:
                            st.markdown("<div class='gc-alert gc-alert-warn'>No procedures parsed yet. Click <b>Parse &amp; Generate Checklists</b> below.</div>", unsafe_allow_html=True)
                        else:
                            from collections import defaultdict as _ddf2
                            by_owner = _ddf2(list)
                            for wt in task_list: by_owner[wt["owner"]].append(wt)
                            for owner_key, owner_tasks in by_owner.items():
                                oinfo = D["users"].get(owner_key,{})
                                st.markdown(f"""<div class='cl-owner-card'>
                                  <div style='font-size:12px;color:#2563eb;font-weight:700;text-transform:uppercase;letter-spacing:.06em;margin-bottom:.2rem'>{oinfo.get('title','')} — {oinfo.get('name',owner_key)}</div>
                                  <div style='font-size:12px;color:#6b7280;margin-bottom:.5rem'>{len(owner_tasks)} procedure(s)</div>""", unsafe_allow_html=True)
                                for wt in owner_tasks:
                                    pc = "badge-high" if wt["category"] in ["AML","Regulatory"] else "badge-med"
                                    st.markdown(f"""<div class='cl-task-row'>
                                      <div style='display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:.3rem'>
                                        <div style='font-weight:600;font-size:13px;color:#111'>{wt['title']}</div>
                                        <span class='gc-badge {pc}'>{wt['category']}</span>
                                      </div>
                                      <div style='font-size:12px;color:#1d4ed8;font-weight:600;margin:.2rem 0'>🔁 {wt['frequency']}</div>
                                      <div style='font-size:12px;color:#6b7280'>{wt['description']}</div>
                                    </div>""", unsafe_allow_html=True)
                                st.markdown("</div>", unsafe_allow_html=True)

                        # Parse / regenerate checklists
                        ...
                        if existing_cls:
                            st.markdown(f"<div class='gc-alert gc-alert-ok'>✓ {len(existing_cls)} active checklist(s) from v{doc['version']}. Auto-updates on new version approval.</div>", unsafe_allow_html=True)
                            if st.button(f"🔄 Re-Parse & Regenerate — {doc['id']}", key=f"regen_{id_prefix}_{doc['id']}"):
                                n = _auto_parse_wsp(doc)
                                add_audit("Policies & WSPs", f"{doc['id']}: checklists regenerated ({n} owner(s))")
                                st.success(f"✓ Regenerated for {n} person(s)."); st.rerun()
                        elif task_list:
                            if st.button(f"⚙️ Parse & Generate Checklists — {doc['id']}", key=f"gen_cl_{id_prefix}_{doc['id']}"):
                                n = _auto_parse_wsp(doc)
                                add_audit("Policies & WSPs", f"{doc['id']}: {n} checklist(s) generated")
                                st.success(f"✓ {n} checklist(s) generated."); st.rerun()

                        # Close row button
                        if st.button(f"▲ Collapse {doc['id']}", key=f"col_{id_prefix}_{doc['id']}"):
...
def page_checklists():
    ...
    if not checklists:
        st.markdown("<div class='gc-alert gc-alert-warn'>No checklists generated yet. Go to <b>Policies &amp; WSPs</b> → open any approved policy → click <b>Parse &amp; Generate Checklists</b>. Checklists are also generated automatically when a new policy version is approved.</div>", unsafe_allow_html=True)
        return
    ...
    for cl in checklists:
        ...
        if toggle(f"cl_{cl['id']}", f"{cl['id']} | {cl['owner_name']} | {cl['wsp_title']}{ver_tag}{lob_tag} ({done}/{total} done)"):
            ...
            for idx, item in enumerate(cl["items"]):
                ...
                border_col   = "#22c55e" if sup_done else "#f59e0b" if item["checked_off"] else "#3b82f6"

                st.markdown(f"""
                <div class='cl-task-row' style='border-left:3px solid {border_col}'>
                  <div style='display:flex;justify-content:space-between;align-items:flex-start'>
                    <div style='flex:1'>
                      <div style='font-weight:700;font-size:14px;margin-bottom:.2rem'>{status_icon} &nbsp;{item['title']}</div>
                      <div style='font-size:12px;color:#6b7280;margin-bottom:.3rem'>🔁 {item['frequency']} &nbsp;·&nbsp; <span class='gc-badge {cat_cls}'>{item['category']}</span> &nbsp;·&nbsp; Policy: {cl['source_wsp']}{ver_tag}</div>
                ...
                if item["evidence"]:
                    ...
                    for ev in item["evidence"]:
                        imm_badge  = "<span class='gc-badge badge-closed'>IMMUTABLE</span>"
                        ev_notes_txt = f" · <i style='color:#6b7280'>{ev['notes']}</i>" if ev.get("notes") else ""
                        st.markdown(f"<div class='gc-tl'>📎 {ev['name']} · {ev['uploaded_by']} · {ev['ts']} {imm_badge}{ev_notes_txt}</div>", unsafe_allow_html=True)
                    st.markdown("</div>", unsafe_allow_html=True)

                st.markdown("</div>", unsafe_allow_html=True)

                # ── Completion controls (assigned user) ──
                if is_mine and not item["checked_off"]:
                    ci1, ci2 = st.columns([2,1])
                    up = ci1.file_uploader(f"Upload evidence for: {item['title'][:40]}", key=f"cl_up_{cl['id']}_{idx}")
                    ...
                    if ci2.button("✅ Mark Complete & Send for Review", key=f"cl_done_{cl['id']}_{idx}"):
                        now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
                        ...
                        cl["audit"].append({"ts":now_str,"user":st.session_state.current_user,"action":f"'{item['title']}' marked complete, awaiting supervisor review"})
                ...
                # ── Supervisor review (completed items awaiting sign-off) ──
                elif item["checked_off"] and not sup_done:
                    comp_user = item["audit"][-1]["user"] if item.get("audit") else "—"
                    st.markdown(f"<div style='font-size:12px;color:#92400e;background:#fffbeb;border-radius:4px;padding:.3rem .6rem;margin:.3rem 0'>⏳ Completed by <b>{comp_user}</b> on {item.get('completed_ts','')[:16]} — awaiting supervisor sign-off</div>", unsafe_allow_html=True)
                    if is_supervisor:
                        sv1, sv2 = st.columns([2,1])
                        sv_notes = sv1.text_area("Supervisor review notes", height=60, key=f"sv_notes_{cl['id']}_{idx}", placeholder="Add review notes, approval comments, or any follow-up required.")
                        if sv2.button("✅ Supervisor Sign-Off", key=f"sv_done_{cl['id']}_{idx}"):
                            now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
                ...
                # ── Fully signed off ──
                    ...
                    st.markdown(f"<div style='font-size:12px;color:#166534;margin:.3rem 0 .5rem 0'>✅ Complete · Supervisor sign-off: <b>{sv_name}</b> · {item.get('supervisor_reviewed_ts','')[:16]}" + (f" · <i>{item['supervisor_notes']}</i>" if item.get('supervisor_notes') else "") + "</div>", unsafe_allow_html=True)
...
def page_evidence():
    ...
    for ev in results:
        ...
        border_col    = "#d1d5db" if ev["archived"] else "#3b82f6"
        arch_html     = "&nbsp;<span class='gc-badge badge-pending'>ARCHIVED</span>" if ev["archived"] else ""
        notes_html    = f"<div style='font-size:12px;color:#374151;margin-top:.3rem'>💬 <i>{ev['notes']}</i></div>" if ev.get("notes") else ""

        st.markdown(f"""
        <div style='background:#ffffff;border:1px solid #e5e7eb;border-left:3px solid {border_col};
          ...
          <div style='display:flex;justify-content:space-between;align-items:flex-start;gap:.5rem;flex-wrap:wrap'>
            <div>
              ...
              <span style='font-weight:700;font-size:13px;margin-left:.5rem'>📎 {ev['filename']}</span>{arch_html}
            </div>
