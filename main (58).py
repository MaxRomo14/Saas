...
def page_ai_copilot():
    ...
    if not st.session_state.copilot_history or st.session_state.copilot_show_prompts:
        ...
        if st.session_state.copilot_show_prompts:
            st.markdown("<div style='font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.1em;margin-bottom:.5rem'>SUGGESTED PROMPTS</div>", unsafe_allow_html=True)

            # Render all prompts as clean inline text chips grouped by category
            prompt_fired = None
            for cat in PROMPT_CATEGORIES:
                st.markdown(f"""
                <div style='font-size:9px;font-weight:800;color:{cat["color"]};text-transform:uppercase;
                     letter-spacing:.09em;margin:.4rem 0 .25rem 0'>{cat["label"]}</div>""",
                unsafe_allow_html=True)
                # render each prompt as a button styled as a subtle text chip
                for pi, (prompt_text, icon) in enumerate(cat["prompts"]):
                    btn_key = f"copilot_sug_{PROMPT_CATEGORIES.index(cat)}_{pi}"
                    short = f"{icon}  {prompt_text[:80]}{'…' if len(prompt_text)>80 else ''}"
                    if st.button(short, key=btn_key):
                        prompt_fired = prompt_text

            # Document review chips
            st.markdown("""
            <div style='font-size:9px;font-weight:800;color:#b45309;text-transform:uppercase;
                 letter-spacing:.09em;margin:.4rem 0 .25rem 0'>📎 DOCUMENT REVIEW (attach a file below)</div>
            <div style='color:#6b7280;font-size:12px;line-height:2'>
              <span style='font-style:italic'>"Review this marketing deck using FINRA/SEC guidance and flag any compliance issues"</span><br/>
              <span style='font-style:italic'>"Review this policy against its regulations — identify amended rules, what changed, and what updates are required"</span><br/>
              <span style='font-style:italic'>"Summarize this document and identify any compliance gaps"</span>
            </div>""", unsafe_allow_html=True)
