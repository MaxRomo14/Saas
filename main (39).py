...
/* Ensure stButton wrapper doesn't force full width */
.stButton {
  display: inline-flex !important;
...
/* ── Small toolbar buttons (expand/collapse all) ── */
...
def _auto_parse_wsp(wsp):
    """Called on doc approval or first expand. Reads wsp_tasks or policy_tasks for this doc
    and upserts supervisory checklists — one per unique procedure owner.
    If checklists already exist for this doc, removes old ones and regenerates."""
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
    doc_id  = wsp["id"]
    # Determine which task dict to use based on ID prefix
    if doc_id.startswith("POL-"):
        task_list = D.get("policy_tasks",{}).get(doc_id, [])
    else:
        task_list = D.get("wsp_tasks",{}).get(doc_id, [])
    if not task_list:
        return 0

    # Remove any existing checklists for this doc (version update)
    D["supervisory_checklists"] = [c for c in D.get("supervisory_checklists",[]) if c.get("source_wsp") != doc_id]

    # Group tasks by owner — one checklist per owner
    owners = {}
    for wt in task_list:
        o = wt["owner"]
    ...
    for owner_key, tasks in owners.items():
        ...
        for wt in tasks:
            cl_items.append({
                ...
                "supervisor_notes":"",
                "audit":[{"ts":now_str,"user":"system","action":f"Task generated from {doc_id} v{wsp['version']} on approval"}],
            })
        D["supervisory_checklists"].append({
            "id":cl_id,"source_wsp":doc_id,"wsp_title":wsp["title"],
        ...
        for grp_label, grp_docs in groups:
            ...
            for i, doc in enumerate(grp_docs):
                ...
                if is_expanded:
                    ...
                    # Evidence = only files uploaded to checklist items for this doc
                    ev_count   = sum(len(item.get("evidence",[])) for c in checklists for item in c.get("items",[]))
                    wf_linked  = [w for w in D.get("workflow_instances",[])
                                  if any(r in w.get("regulation","") for r in regs[:1])]
                    ...
                    if not rel_tests:
                        ...
                        for t in rel_tests:
                            ...
                            last_ts  = t.get("completed_ts") or t.get("date_started") or t.get("created","")
                            last_ts  = last_ts[:10] if last_ts else "—"
                            exc_ct   = len(t.get("exceptions",[]))
                            exc_str  = f" · <span style='color:#dc2626;font-weight:700'>{exc_ct} exception(s)</span>" if exc_ct else ""
                            st.markdown(f"""
                            <div style='background:{t_bg};border:1px solid #e5e7eb;border-left:3px solid {t_color};
                                 border-radius:6px;padding:.4rem .8rem;margin-bottom:4px;
                                 display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.4rem'>
                              <div>
                                <div style='font-size:12px;font-weight:700;color:#111827'>{t["id"]} — {t["topic"]}</div>
                                <div style='font-size:11px;color:#6b7280'>{t["test_type"]} &nbsp;·&nbsp; Last activity: {last_ts}{exc_str}</div>
                              </div>
                              <span style='background:{t_color}22;color:{t_color};font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;white-space:nowrap'>{t_status}</span>
                            </div>
                            """, unsafe_allow_html=True)
