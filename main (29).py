...
def page_wsp():
    ...
    with tab1:
        ...
        for wsp in approved:
            ...
            wsp_doc_text = "\n".join(wsp_doc_lines)
            wsp_b64      = _b64w.b64encode(wsp_doc_text.encode()).decode()
            wsp_fname    = f"{wsp['id']}_v{wsp['version']}.txt"

            # ── WSP row card ──
            st.markdown(f"""
            <div style='background:#ffffff;border:1px solid #e5e7eb;border-left:4px solid #22c55e;
                 border-radius:8px;padding:.75rem 1.1rem;margin-bottom:4px;
                 display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.5rem'>
              <div>
                <div style='display:flex;align-items:center;gap:.6rem'>
                  <span style='font-size:11px;font-weight:800;color:#22c55e'>{wsp['id']}</span>
                  <span style='font-size:13px;font-weight:700;color:#111111'>{wsp['title']}</span>
                  <span style='background:#dcfce7;color:#166534;font-size:10px;font-weight:700;padding:1px 8px;border-radius:10px'>v{wsp['version']}</span>
                </div>
            ...
            # ── Details toggle ──
            if toggle(f"wsp_{wsp['id']}", f"📋  Details — {wsp['id']}"):
                st.markdown(f"""
