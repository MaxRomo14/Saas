...
/* ── Download Original Request button — white text ── */
.dl-req-btn > div > button,
.dl-req-btn button {
...
.dl-req-btn > div > button:hover,
.dl-req-btn button:hover {
    ...
    with tab1:
        for er in D["exam_requests"]:
            ...
            if toggle(f"er_{er['id']}", f"{er['id']} — {er['title']} | {er['regulator']} | Due: {er['due']}"):
                ...
                st.markdown("<div class='dl-req-btn'>", unsafe_allow_html=True)
                st.download_button(
...
def page_evidence():
    ...
    # Topic = task category
    topics      = sorted(set(D["tasks"][idx]["category"] for idx in range(len(D["tasks"])) for ev in ev_list if ev.get("task_id")==D["tasks"][idx]["id"]), default=[]) if D["tasks"] else []
