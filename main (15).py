...
[data-testid="stExpander"] details > div * { color: #111111 !important; }
.streamlit-expanderHeader { background-color: #ffffff !important; }
.streamlit-expanderHeader svg { display: none !important; }
.streamlit-expanderHeader p { font-size: 13px !important; font-weight: 600 !important; color: #111111 !important; }
.streamlit-expanderContent { background-color: #f8f9fa !important; }
.streamlit-expanderContent * { color: #111111 !important; }
...
# ── JS: Remove SVG arrow elements that bleed "arrow_right" text into DOM ──
st.markdown("""
<script>
(function removeSVGArrows() {
  function clean() {
    // Find every SVG inside expander summaries and remove it from the DOM
    var summaries = document.querySelectorAll('[data-testid="stExpander"] summary, .streamlit-expanderHeader');
    summaries.forEach(function(summary) {
      var svgs = summary.querySelectorAll('svg');
      svgs.forEach(function(svg) { svg.remove(); });
    });
    // Also remove any stray text nodes that say arrow_right or arrow_down
    var walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, null, false);
    var node;
    var toRemove = [];
    while (node = walker.nextNode()) {
      if (node.nodeValue && (node.nodeValue.includes('arrow_right') || node.nodeValue.includes('arrow_down'))) {
        toRemove.push(node);
      }
    }
    toRemove.forEach(function(n) { n.parentNode && n.parentNode.removeChild(n); });
  }
  // Run on load and observe DOM mutations (Streamlit re-renders)
  clean();
...
def page_wsp():
    ...
    with tab1:
        for wsp in D["wsps"]:
            ...
            with st.expander(f"{wsp['id']} — {wsp['title']}  (v{wsp['version']})  [{wsp['status']}]"):
...
def page_checklists():
    ...
    for cl in checklists:
        ...
        with st.expander(f"{cl['id']} | {cl['owner_name']} | {cl['wsp_title']} ({done}/{total} done)"):
            # Header info
            st.markdown(f"""
