...
def page_wsp():
    ...
    def _render_library(docs, tasks_key, id_prefix):
        ...
        ctrl1, ctrl2, ctrl3, ctrl4 = st.columns([1, 1, 1.3, 5])
        if ctrl1.button("⊞ All", key=f"{id_prefix}_expall"):
            for w in approved: st.session_state[f"docrow_{id_prefix}_{w['id']}"] = True
            st.rerun()
        if ctrl2.button("⊟ None", key=f"{id_prefix}_colall"):
            for w in approved: st.session_state[f"docrow_{id_prefix}_{w['id']}"] = False
            st.rerun()
        f_view = ctrl3.selectbox("View", ["Grouped by LOB","Flat list"], key=f"{id_prefix}_view", label_visibility="collapsed")
        ...
        ctrl4.markdown(f"<div style='font-size:11px;color:#6b7280;padding:.42rem 0'>{len(filtered)} of {len(approved)} document(s)</div>", unsafe_allow_html=True)
            ...
            for i, doc in enumerate(grp_docs):
                ...
                if is_expanded:
                    ...
                    def _chain_node(icon, label, count, color, bg, nav_hint=""):
                        count_str = f"<div style='font-size:16px;font-weight:800;color:{color}'>{count}</div>" if count != "" else ""
                        hint_str  = f"<div style='font-size:9px;color:#9ca3af;margin-top:1px'>{nav_hint}</div>" if nav_hint else ""
                        return f"""
                        <div style='background:{bg};border:1px solid {color}33;border-radius:8px;
                             padding:.45rem .7rem;text-align:center;min-width:90px'>
                          <div style='font-size:14px'>{icon}</div>
                          {count_str}
                          <div style='font-size:9px;font-weight:700;color:{color};text-transform:uppercase;letter-spacing:.04em'>{label}</div>
                          {hint_str}
                        </div>"""
                    def _arrow():
                        return "<div style='font-size:18px;color:#cbd5e1;align-self:center;padding:0 .1rem'>→</div>"

                    node1 = _chain_node("📄", doc_type,        "",          "#7c3aed" if doc_type=="Policy" else "#0369a1", "#f3e8ff" if doc_type=="Policy" else "#e0f2fe")
                    node2 = _chain_node("📋", "Procedures",    proc_count,  "#2563eb","#eff6ff", "Supervisory")
                    node3 = _chain_node("✅", "Checklists",    len(checklists),"#059669","#f0fdf4","Checklists tab")
                    node4 = _chain_node("📁", "Evidence",      ev_count,    "#d97706","#fffbeb","Evidence Repo")
                    node5 = _chain_node("🧪", "Tests",         test_count,  "#7c3aed","#faf5ff","Comp. Testing")
                    node6 = _chain_node("⚠️", "Remediation",   rem_count,   "#dc2626","#fef2f2","Rem. Items")

                    st.markdown(f"""
                    <div style='display:flex;align-items:stretch;gap:.3rem;margin-bottom:.8rem;flex-wrap:wrap'>
                      {node1}{_arrow()}{node2}{_arrow()}{node3}{_arrow()}{node4}{_arrow()}{node5}{_arrow()}{node6}
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    if not task_list:
                        st.markdown("<div class='gc-alert gc-alert-warn' style='font-size:12px'>No procedures parsed yet. Click <b>Parse &amp; Generate Checklists</b> below.</div>", unsafe_allow_html=True)
                    else:
                        for wt in task_list:
                    ...
                    pb1, pb2, pb3 = st.columns([2, 2, 4])
                    if existing_cls:
                        st.markdown(f"<div class='gc-alert gc-alert-ok' style='font-size:12px'>✓ {len(existing_cls)} active checklist(s) from v{doc['version']}. Auto-updates on approval.</div>", unsafe_allow_html=True)
                        if pb1.button(f"🔄 Re-Parse — {doc['id']}", key=f"regen_{id_prefix}_{doc['id']}"):
                            n = _auto_parse_wsp(doc)
                            add_audit("Policies & WSPs", f"{doc['id']}: checklists regenerated ({n})")
                            st.success(f"✓ Regenerated for {n} person(s)."); st.rerun()
                    elif task_list:
                        if pb1.button(f"⚙️ Generate Checklists — {doc['id']}", key=f"gen_cl_{id_prefix}_{doc['id']}"):
                            n = _auto_parse_wsp(doc)
                            add_audit("Policies & WSPs", f"{doc['id']}: {n} checklist(s) generated")
                            st.success(f"✓ {n} checklist(s) generated."); st.rerun()
                    if pb2.button(f"▲ Collapse", key=f"col_{id_prefix}_{doc['id']}"):
