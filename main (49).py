...
def page_dashboard():
    ...
    # Compliance Team doesn't see Pending Approval (CCO queue) metric
    if is_compliance_team():
        cols[2].metric("My Tasks", len([t for t in tasks if t["assignee"]==st.session_state.current_user]))
    else:
...
def page_tasks():
    ...
    with tab1:
        ...
        fs=fc1.selectbox("Status", status_opts)
