...
def page_wsp():
    st.markdown("<div class='gc-header'><h1>POLICIES & WSPs</h1><p>Policy library organized by line of business — folder view, version control, and automatic checklist generation on approval</p></div>", unsafe_allow_html=True)
    ...
    tab1, tab2, tab3 = st.tabs(["📁  Policy Library","🕐  Drafts Pending Review","➕  New Policy / WSP"])

    # ══════════════════════════════════════════════════════════════
    # TAB 1 — FOLDER VIEW (Line of Business → Policies)
    # ══════════════════════════════════════════════════════════════
    with tab1:
        ...
        if not approved:
            st.markdown("<div class='gc-alert gc-alert-warn'>No approved policies on file.</div>", unsafe_allow_html=True)

        # ── LOB filter ──
        lobs_present = sorted(set(w.get("line_of_business","Operations") for w in approved))
        fc1, fc2 = st.columns([2,2])
        f_lob    = fc1.selectbox("Filter by Line of Business", ["All"] + lobs_present)
        f_search = fc2.text_input("🔍 Search policies", placeholder="title, ID, owner...")

        if f_lob    != "All": approved = [w for w in approved if w.get("line_of_business") == f_lob]
        if f_search:
            q = f_search.lower()
            approved = [w for w in approved if q in w["title"].lower() or q in w["id"].lower() or q in w.get("owner","").lower()]
        ...
        for lob in sorted(by_lob.keys()):
            ...
            <div style='background:#1e293b;color:#ffffff;border-radius:8px;
              ...
              <span style='font-size:14px;font-weight:800;letter-spacing:.05em'>📁 {lob.upper()}</span>
              <span style='font-size:12px;color:#94a3b8'>{len(lob_wsps)} policy/WSP(s)</span>
            </div>
            """, unsafe_allow_html=True)
            ...
            for wsp in lob_wsps:
                ...
                task_count    = len(D["wsp_tasks"].get(wsp["id"],[]))
                cl_count      = len([c for c in D.get("supervisory_checklists",[]) if c.get("source_wsp")==wsp["id"]])

                # Build download doc
                wsp_doc_lines = [
                    f"{'='*60}",
                    f"  {wsp['id']} — {wsp['title']}",
                    f"  Line of Business: {wsp.get('line_of_business','')}",
                    f"  Version: {wsp['version']}  |  Status: {wsp['status']}",
                    f"  Owner: {owner_name}",
                    f"  Approved By: {approver_name}  |  Date: {wsp['approved_ts'][:10]}",
                    f"{'='*60}", "",
                    "POLICY CONTENT", "-"*60,
                    wsp.get("content",""), "",
                    "VERSION HISTORY", "-"*60,
                ]
                for v in wsp.get("versions",[]):
                    wsp_doc_lines.append(f"  v{v['ver']}  {v['ts']}  {v['author']}  —  {v['note']}")
                wsp_doc_lines += ["","AUDIT TRAIL","-"*60]
                for e in wsp.get("audit",[]):
                    wsp_doc_lines.append(f"  {e['ts']}  {e['user']}  {e['action']}")
                wsp_b64  = _b64w.b64encode("\n".join(wsp_doc_lines).encode()).decode()
                wsp_fname = f"{wsp['id']}_v{wsp['version']}.txt"
                ...
                <div style='background:#ffffff;border:1px solid #e5e7eb;border-left:4px solid #22c55e;
                  ...
                  <div style='min-width:0;flex:1'>
                    <div style='display:flex;align-items:center;gap:.5rem;flex-wrap:wrap'>
                      <span style='font-size:11px;font-weight:800;color:#22c55e;white-space:nowrap'>{wsp['id']}</span>
                      <span style='font-size:13px;font-weight:700;color:#111111'>📄 {wsp['title']}</span>
                      <span style='background:#dcfce7;color:#166534;font-size:10px;font-weight:700;padding:1px 8px;border-radius:10px;white-space:nowrap'>v{wsp['version']}</span>
                    </div>
                    <div style='font-size:11px;color:#6b7280;margin-top:3px'>
                      Owner: <b>{owner_name}</b> &nbsp;·&nbsp; Approved by: <b>{approver_name}</b> &nbsp;·&nbsp; {wsp['approved_ts'][:10]}
                      &nbsp;·&nbsp; <span style='color:#2563eb'>{task_count} supervisory procedure(s)</span>
                      &nbsp;·&nbsp; <span style='color:#7c3aed'>{cl_count} active checklist(s)</span>
                    </div>
                  </div>
                  <a href="data:text/plain;base64,{wsp_b64}" download="{wsp_fname}"
                ...
                # ── Details + Parse toggle ──
                if toggle(f"wsp_det_{wsp['id']}", f"📋  Details & Supervisory Procedures — {wsp['id']}"):
                    ...
                    # ── Supervisory procedures parsed from this WSP ──
                    ...
                    if not wsp_task_list:
                        st.markdown("<div class='gc-alert gc-alert-warn'>No supervisory procedures parsed yet. Click <b>Parse & Generate Checklists</b> below to extract procedures and assign to responsible persons.</div>", unsafe_allow_html=True)
                    else:
                        # Group by owner
                        owners = {}
                        ...
                        for owner_key, tasks_for_owner in owners.items():
                            ...
                            <div class='cl-owner-card'>
                              <div style='font-size:12px;color:#2563eb;font-weight:700;text-transform:uppercase;letter-spacing:.06em;margin-bottom:.2rem'>
                                {oinfo.get('title',oinfo.get('role',''))} — {oinfo.get('name',owner_key)}
                              </div>
                              <div style='font-size:12px;color:#6b7280;margin-bottom:.6rem'>{len(tasks_for_owner)} procedure(s) assigned</div>
                            """, unsafe_allow_html=True)
                            for wt in tasks_for_owner:
                                freq_col = "#166534" if "Daily" in wt["frequency"] else "#92400e" if "Weekly" in wt["frequency"] else "#1e40af"
                                ...
                                <div class='cl-task-row'>
                                  <div style='display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:.3rem'>
                                    <div style='font-weight:600;font-size:13px'>{wt['title']}</div>
                                    <span class='gc-badge {pc}'>{wt['category']}</span>
                                  </div>
                                  <div style='font-size:12px;color:{freq_col};font-weight:600;margin:.2rem 0'>🔁 {wt['frequency']}</div>
                                  <div style='font-size:12px;color:#6b7280'>{wt['description']}</div>
                                </div>
                                """, unsafe_allow_html=True)
                            st.markdown("</div>", unsafe_allow_html=True)

                    # ── Parse & generate / re-generate checklists ──
                    st.markdown("<hr/>", unsafe_allow_html=True)
                    existing_cls = [c for c in D.get("supervisory_checklists",[]) if c.get("source_wsp")==wsp["id"]]
                    if existing_cls:
                        st.markdown(f"<div class='gc-alert gc-alert-ok'>✓ {len(existing_cls)} active checklist(s) generated from this policy (v{wsp['version']}). Checklists update automatically when a new version is approved.</div>", unsafe_allow_html=True)
                        if st.button(f"🔄 Re-Parse & Regenerate Checklists — {wsp['id']}", key=f"regen_{wsp['id']}"):
                            n = _auto_parse_wsp(wsp)
                            add_audit("Policies & WSPs", f"{wsp['id']}: checklists regenerated ({n} owner(s))")
                            st.success(f"✓ Checklists regenerated for {n} responsible person(s)."); st.rerun()
                    else:
                        if wsp_task_list:
                            if st.button(f"⚙️ Parse & Generate Checklists from {wsp['id']}", key=f"gen_cl_{wsp['id']}"):
                                n = _auto_parse_wsp(wsp)
                                add_audit("Policies & WSPs", f"{wsp['id']}: parsed and {n} checklist(s) generated")
                                st.success(f"✓ {n} checklist(s) generated and assigned."); st.rerun()
    ...
    with tab2:
        ...
        for wsp in drafts:
            ...
            if toggle(f"wsp_draft_{wsp['id']}", f"{wsp['id']} — {wsp['title']}  ·  v{wsp['version']}  ·  {wsp.get('line_of_business','')}  ·  {rs_lbl}"):
                ...
                <div style='background:{rs_bg};border:1px solid #e5e7eb;border-radius:8px;
                  ...
                  <div style='text-align:right'>
                    ...
                    {'<div style="font-size:11px;color:#6b7280;margin-top:.2rem">Sent: '+wsp["review_sent_ts"][:16]+'</div>' if wsp.get("review_sent_ts") else ''}
                  </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                if wsp.get("review_status") not in ("approved",):
                    ...
                    new_fnotes = uf2.text_area("File Notes / Context", height=90, key=f"wsp_fnotes_{wsp['id']}", placeholder="e.g. Redline showing tracked changes to section 3.")
                    if new_file:
                        wsp["files"].append({"name":new_file.name,"type":new_ftype,"uploaded_by":st.session_state.current_user,"ts":datetime.now().strftime("%Y-%m-%d %H:%M"),"notes":new_fnotes.strip()})
                        now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
                        ...
                        add_audit("Policies & WSPs", f"{wsp['id']}: {new_ftype} file uploaded")
                ...
                if wsp.get("review_status") in ("with_author","pending_changes"):
                    ...
                    sel_rev  = sr1.selectbox("Send To", rev_opts, format_func=lambda x: f"{D['users'][x]['name']} ({D['users'][x]['title']})", key=f"wsp_rev_{wsp['id']}")
                    rev_note = sr2.text_area("Message to Reviewer", height=68, key=f"wsp_revnote_{wsp['id']}", placeholder="Optional note for the reviewer.")
                    if st.button(f"📤 Send {wsp['id']} for Review", key=f"wsp_send_{wsp['id']}"):
                        if not wsp["files"]:
                            ...
                            now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
                            wsp["reviewer"] = sel_rev; wsp["review_sent_ts"] = now_str; wsp["review_status"] = "with_reviewer"
                            ...
                            add_audit("Policies & WSPs", f"{wsp['id']} sent for review")
                            st.success(f"✓ Sent to {D['users'][sel_rev]['name']}."); st.rerun()

                # Reviewer actions — approve triggers auto-parse
                ...
                if wsp.get("review_status") == "with_reviewer" and is_reviewer:
                    ...
                    rv_comments = st.text_area("Comments / Review Notes", height=80, key=f"wsp_rvcomm_{wsp['id']}", placeholder="Approval notes or required changes.")
                    rva, rvb, _ = st.columns(3)
                    if rva.button(f"✅ Approve {wsp['id']}", key=f"wsp_approve_{wsp['id']}"):
                        now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
                        wsp["status"] = "Approved"; wsp["review_status"] = "approved"
                        wsp["approved_by"] = st.session_state.current_user; wsp["approved_ts"] = now_str
                        wsp["audit"].append({"ts":now_str,"user":st.session_state.current_user,"action":"Policy approved" + (f" — {rv_comments.strip()}" if rv_comments.strip() else "")})
                        ...
                        # ── AUTO-PARSE on approval ──
                        n = _auto_parse_wsp(wsp)
                        add_audit("Policies & WSPs", f"Approved {wsp['id']} v{wsp['version']}; {n} checklist(s) auto-generated")
                        st.success(f"✓ {wsp['id']} approved. {n} supervisory checklist(s) automatically generated and assigned."); st.rerun()
                    if rvb.button(f"↩ Return — Pending Changes", key=f"wsp_return_{wsp['id']}"):
                        now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
                        wsp["review_status"] = "pending_changes"
                        wsp["audit"].append({"ts":now_str,"user":st.session_state.current_user,"action":"Returned — pending changes" + (f": {rv_comments.strip()}" if rv_comments.strip() else "")})
                        add_audit("Policies & WSPs", f"{wsp['id']} returned for revision"); st.rerun()
    ...
    # ══════════════════════════════════════════════════════════════
    # TAB 3 — NEW POLICY / WSP
    # ══════════════════════════════════════════════════════════════
    with tab3:
        st.markdown("<div style='font-size:13px;color:#6b7280;margin-bottom:1.2rem'>Upload your drafted policy or WSP here. Use Word or your preferred editor for the document — upload the redline and clean copy, select the line of business, then send for review and approval. Checklists are automatically generated when the policy is approved.</div>", unsafe_allow_html=True)

        nc1, nc2, nc3 = st.columns(3)
        new_title = nc1.text_input("Policy / WSP Title *", placeholder="e.g. Supervisory Procedures – Fixed Income")
        new_lob   = nc2.selectbox("Line of Business *", LINES_OF_BUSINESS)
        new_ver   = nc3.text_input("Version *", value="1.0")

        fu1, fu2 = st.columns([1,1])
        new_up    = fu1.file_uploader("Browse and select file", key="new_wsp_up")
        ...
        if st.button("💾 Save as Draft", key="save_new_wsp"):
            if not new_title.strip():
                ...
                D["wsps"].append({
                    ...
                    "line_of_business":new_lob,
                    "owner":st.session_state.current_user,
                    "approved_by":"","approved_ts":"","content":"",
                ...
                st.success(f"✓ {nid} saved as Draft under {new_lob}. Go to the Drafts tab to upload more files or send for review.")
                st.rerun()
