            ...
            if toggle(f"er_{er['id']}", f"{er['id']} — {er['title']} | {er['regulator']} | Due: {er['due']}"):
                ...
                for idx,item in enumerate(er["items"]):
                    ...
                    if up:
                        ...
                        er["audit"].append({"ts":datetime.now().strftime("%Y-%m-%d %H:%M"),"user":st.session_state.current_user,"action":f"Uploaded {up.name} for '{item['item']}'" + (f" — {er_notes.strip()}" if er_notes.strip() else "")})
...
def page_evidence():
    ...
    for ev in ev_list:
        ...
        st.markdown(f"""<div class='gc-card'>
          ...
          {f"<div style='margin-top:.3rem;font-size:12px;color:#374151'>💬 <i>{ev['notes']}</i></div>" if ev.get('notes') else ''}
        </div>""", unsafe_allow_html=True)
    st.markdown("<hr/>", unsafe_allow_html=True)
    c1,c2=st.columns(2)
    tid=c1.selectbox("Link to Task",[t["id"] for t in D["tasks"]])
    up=c2.file_uploader("Upload File")
    ev_notes = st.text_area("Upload notes / context", key="ev_repo_notes", height=68, placeholder="Describe what this file is, why it's being uploaded, any issues, exceptions, or context.")
    if up:
        fh="sha256:"+hashlib.md5(up.name.encode()).hexdigest()
        eid=f"EV-{len(D['evidence'])+1:03d}"
        st.session_state.data["evidence"].append({"id":eid,"filename":up.name,"uploaded_by":st.session_state.current_user,"ts":datetime.now().strftime("%Y-%m-%d %H:%M"),"task_id":tid,"hash":fh,"immutable":True,"archived":False,"notes":ev_notes.strip()})
        add_audit("Evidence",f"Uploaded {up.name} → {tid}" + (f" — {ev_notes.strip()}" if ev_notes.strip() else "")); st.success(f"✓ {up.name} stored as {eid} (WORM-locked)")
    ...
    # ──────────────────────────────────────────────────────────────────────────
        ...
        with tt2:
            ...
            for idx, proc in enumerate(test["procedures"]):
                ...
                res_col = {"Pass":"#166534","Fail":"#991b1b","Exception":"#854d0e","N/A":"#6b7280","":"#6b7280"}.get(proc["result"],"#6b7280")
                ...
                <div style='background:{res_bg};border:1px solid #e5e7eb;{exc_border};
                  ...
                  <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:.5rem'>
                    ...
                    <div style='display:flex;gap:.5rem;align-items:center'>
                      {'<span style="background:#fffbeb;color:#92400e;font-size:10px;padding:1px 8px;border-radius:10px;font-weight:700">⚠ EXCEPTION</span>' if has_exc else ''}
                      {_ct_result_badge(proc['result'])}
                    </div>
                  </div>
                  <div style='font-size:13px;color:#111111;font-weight:500;margin-bottom:.5rem'>{proc['description']}</div>
                  {'<div style="font-size:12px;color:#6b7280;margin-top:.3rem">Tested by: <b>'+D["users"].get(proc["tested_by"],{}).get("name","—")+'</b> · '+proc["tested_ts"]+'</div>' if proc["tested_ts"] else ''}
                </div>
                """, unsafe_allow_html=True)
            ...
            for ex in exceptions:
                ...
                <div style='background:#fffbeb;border:1px solid #fde68a;border-left:3px solid #f59e0b;
                  ...
                  <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:.4rem'>
                    <div style='font-size:12px;font-weight:700;color:#92400e'>{ex['id']} &nbsp;·&nbsp; Step ref: {ex['procedure_id']}</div>
                    {badge(ex['status'], exc_sc)}
                  </div>
                  <div style='font-size:13px;font-weight:600;color:#111111;margin-bottom:.4rem'>{ex['description']}</div>
                  {'<div style="font-size:12px;color:#374151;margin:.2rem 0"><b>Root Cause:</b> '+ex['root_cause']+'</div>' if ex.get('root_cause') else ''}
                  {'<div style="font-size:12px;color:#374151;margin:.2rem 0"><b>Remediation:</b> '+ex['remediation_plan']+'</div>' if ex.get('remediation_plan') else ''}
                  {'<div style="font-size:12px;color:#374151"><b>Assigned to:</b> '+D["users"].get(ex["assigned_to"],{}).get("name","—")+' &nbsp;·&nbsp; <b>Due:</b> '+ex["due_date"]+'</div>' if ex.get('assigned_to') else ''}
                </div>
                """, unsafe_allow_html=True)
                ...
                <div class='gc-card'>
                  ...
                  <div style='display:flex;gap:2rem;margin-bottom:.8rem;flex-wrap:wrap'>
                    ...
                    <div><div style='font-size:11px;color:#6b7280;text-transform:uppercase;font-weight:700'>Reviewer</div>
                      <div style='font-size:13px;font-weight:600;margin-top:.2rem'>{D['users'].get(test.get('reviewer',''),{}).get('name','—')}</div></div>
                  </div>
                  <div style='font-size:13px;line-height:1.7;color:#374151'>{test.get('conclusion','—')}</div>
                  {'<div class="gc-card-header" style="margin-top:.8rem">REVIEW NOTES</div><div style="font-size:13px;color:#374151">'+test["review_notes"]+'</div>' if test.get("review_notes") else ''}
                </div>
                """, unsafe_allow_html=True)
                # Export
