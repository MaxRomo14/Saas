...
/* ── Background: force grey everywhere ── */
.stApp,
...
/* ── ALL text on the grey background must be dark ── */
...
section[data-testid="stSidebar"] > div {
  ...
  border-right: 1px solid #0d0f14 !important;
}
section[data-testid="stSidebar"] *,
...
.stDataFrame tbody td { color: #f1f3f6 !important; font-size: 13px !important; }
...
.gc-card * { color: #f1f3f6 !important; }
.gc-card-header {
  font-size: 11px;
  font-weight: 700;
...
.badge-open    { background: rgba(59,130,246,0.25);  color: #93c5fd !important; }
.badge-closed  { background: rgba(34,197,94,0.25);   color: #86efac !important; }
.badge-overdue { background: rgba(239,68,68,0.25);   color: #fca5a5 !important; }
.badge-pending { background: rgba(251,191,36,0.25);  color: #fde68a !important; }
.badge-high    { background: rgba(239,68,68,0.25);   color: #fca5a5 !important; }
.badge-med     { background: rgba(251,191,36,0.25);  color: #fde68a !important; }
.badge-low     { background: rgba(34,197,94,0.25);   color: #86efac !important; }
...
.gc-timeline-item * { color: var(--text2) !important; }
...
.gc-header p { font-size: 13px; color: var(--text2) !important; margin: 0; }

/* ── Alerts ── */
.gc-alert { border-radius: 6px; padding: 0.7rem 1rem; margin: 0.5rem 0; font-size: 13px; line-height: 1.5; }
.gc-alert-warn   { background: rgba(245,158,11,0.15);  border-left: 3px solid var(--warn);    color: #fcd34d !important; }
.gc-alert-danger { background: rgba(239,68,68,0.15);   border-left: 3px solid var(--danger);  color: #fca5a5 !important; }
.gc-alert-ok     { background: rgba(34,197,94,0.15);   border-left: 3px solid var(--success); color: #86efac !important; }
...
/* ── Success / info / warning native boxes ── */
[data-testid="stAlert"] { border-radius: 6px !important; }

/* ── Login ── */
.login-wrap { max-width: 400px; margin: 6vh auto; }
.login-logo { font-size: 1.8rem; font-weight: 700; color: #1e2128 !important; letter-spacing: -0.02em; text-align: center; margin-bottom: 0.2rem; }
...
.login-wrap .gc-card * { color: #f1f3f6 !important; }
</style>
...
def sidebar():
    with st.sidebar:
        ...
        <div style='padding:0.8rem 0 1rem 0;border-bottom:1px solid var(--border);margin-bottom:1rem'>
          <div style='font-family:var(--display);font-size:1.5rem;color:var(--accent);letter-spacing:0.04em;font-style:italic;font-weight:500'>⚖ Graphite</div>
          <div style='font-size:11px;color:#8899bb;letter-spacing:0.12em;text-transform:uppercase'>Compliance Platform</div>
        </div>
        ...
        <div style='font-size:0.9rem;font-weight:600;color:#111827;margin-bottom:0.2rem'>{current_name()}</div>
        <div style='font-size:0.72rem;color:var(--accent);font-family:var(--mono)'>{current_role()}</div>
        <hr/>
        """, unsafe_allow_html=True)
...
def page_dashboard():
    ...
    with col_l:
        st.markdown("<div class='gc-card-header' style='font-family:var(--mono);font-size:0.75rem;letter-spacing:0.1em;color:var(--accent);text-transform:uppercase;margin-bottom:0.8rem'>MY TASKS</div>", unsafe_allow_html=True)
    ...
    with col_r:
        st.markdown("<div class='gc-card-header' style='font-family:var(--mono);font-size:0.75rem;letter-spacing:0.1em;color:var(--accent);text-transform:uppercase;margin-bottom:0.8rem'>RECENT ACTIVITY</div>", unsafe_allow_html=True)
        for entry in reversed(D["audit_trail"][-8:]):
            st.markdown(f"""
            <div class='gc-timeline-item'>
              <span style='color:var(--accent);font-family:var(--mono);font-size:0.68rem'>{entry['ts']}</span>
              <span style='color:var(--text2)'> · {entry['user']}</span><br/>
              <span style='color:#111827'>{entry['action']}</span>
            </div>
            """, unsafe_allow_html=True)
        ...
        for t in tasks:
            ...
            with st.expander(f"[{t['id']}]  {t['title']}"):
                ...
                for entry in t["audit"]:
                    st.markdown(f"<div class='gc-timeline-item'><span style='color:var(--accent);font-family:var(--mono);font-size:0.68rem'>{entry['ts']}</span>  ·  <b>{entry['user']}</b>  ·  {entry['action']}</div>", unsafe_allow_html=True)
    ...
    with tab1:
        for wsp in D["wsps"]:
            ...
            with st.expander(f"[{wsp['id']}]  {wsp['title']}  ·  v{wsp['version']}"):
                ...
                st.markdown("<hr/>", unsafe_allow_html=True)
                st.markdown("<div class='gc-card-header' style='font-family:var(--mono);font-size:0.72rem;letter-spacing:0.1em;color:var(--accent);text-transform:uppercase;margin-bottom:0.6rem'>📋 Generate Supervisory Checklist from this WSP</div>", unsafe_allow_html=True)
                ...
                for item in suggested:
                    ...
                    <div style='display:flex;justify-content:space-between;align-items:center;
                                ...
                                background:#f8fafc;border:1px solid #e2e5ea;border-radius:4px;font-size:0.8rem'>
...
def page_evidence():
    ...
    for ev in ev_list:
        ...
        <div class='gc-card'>
          <div style='display:flex;justify-content:space-between;align-items:center'>
            <div>
              <span style='font-family:var(--mono);color:var(--accent);font-size:0.72rem'>{ev['id']}</span>  
    ...
    for node in D["org_chart"]:
        ...
        <div class='gc-card'>
          <div style='display:flex;justify-content:space-between;align-items:flex-start'>
            <div>
              <div style='font-family:var(--mono);color:var(--accent);font-size:0.8rem;letter-spacing:0.06em'>{node['role']}</div>
...
def page_audit():
    ...
    for entry in trail:
        st.markdown(f"""
        <div class='gc-timeline-item'>
          <span style='color:var(--accent);font-family:var(--mono);font-size:0.7rem'>{entry['ts']}</span>
