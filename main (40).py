...
/* ── Buttons — uniform micro size everywhere ── */
.stButton > button {
...
/* ── Dropdown / selectbox popover — force white bg dark text everywhere ── */
...
/* The selected value pill inside the closed selectbox */
.stSelectbox [data-baseweb="select"] > div,
...
/* Input inside select search */
[data-baseweb="select"] input {
  color: #111111 !important;
  background: #ffffff !important;
}
/* Override any dark-mode portal that Streamlit injects */
...
def _build_data_lake(ts_fn):
    ...
    LINES_OF_BIZ = ["Wealth Management","Investment Banking","Operations","Retail Brokerage",
                    "Fixed Income","Equity Trading","Investment Advisory","Prime Brokerage"]
    ...
    UPLOADERS = ["admin","supervisor","analyst"]
    ...
    system_files = [
        ("supervision_policy_v2.pdf","analyst","T-004","WSP","Wealth Management","2026","System Upload",["FINRA 3110","Supervision"]),
        ("aml_policy_2024.pdf","admin","T-004","Policy","Operations","2024","System Upload",["AML","BSA"]),
        ("AML_Training_Completion_2025.xlsx","admin","T-001","Training Record","Operations","2025","Task Evidence",["AML","Training"]),
        ...
        ("Best_Execution_Analysis_H1_2025.pdf","admin","T-003","Testing Evidence","Fixed Income","2025","Task Evidence",["Best Execution","Reg BI"]),
        ("Rule_3110_Gap_Analysis.xlsx","analyst","T-004","Audit Report","Compliance","2025","Task Evidence",["FINRA 3110","Annual Review"]),
        ("CT001_BCP_Evidence_Binder.pdf","supervisor","","Exam Evidence","Operations","2026","System Upload",["BCP","Exam Evidence"]),
        ("CT002_Annual_Compliance_Review.pdf","admin","","Testing Evidence","Investment Advisory","2026","System Upload",["SEC 206(4)-7","Annual Review"]),
        ("CCO_Annual_Certification_2025.pdf","admin","","Certification","Compliance","2025","Manual Upload",["Certification","Annual Review"]),
        ("SAR_Review_Log_Q4_2025.xlsx","supervisor","","SAR Filing","Operations","2025","System Upload",["SAR","AML"]),
        ("Info_Barrier_Attestations_2025.zip","analyst","","Certification","Investment Banking","2025","System Upload",["Information Barrier","Certification"]),
        ("Reg_BI_Conflict_Review_Q1_2026.pdf","admin","","Supervisory Review","Wealth Management","2026","Task Evidence",["Reg BI","Supervision"]),
        ("FINRA_Exam_Response_March2026.pdf","admin","","Correspondence","Compliance","2026","Manual Upload",["Exam Evidence","FINRA 3110"]),
        ...
        ("Customer_Complaint_Log_2025.xlsx","analyst","","Complaint File","Retail Brokerage","2025","System Upload",["Complaint"]),
    ]
...
def seed_data():
    ...
    policy_tasks = {
        "POL-001": [
            {"id":"PT-001-1","title":"Annual AML Policy Certification","owner":"admin","role_title":"Chief Compliance Officer","frequency":"Annually","description":"Review and certify the firm's AML Policy is current, reflects applicable FinCEN guidance, and has been distributed to all required personnel.","category":"AML"},
            {"id":"PT-001-2","title":"AML Program Independent Testing Coordination","owner":"admin","role_title":"Chief Compliance Officer","frequency":"Annually","description":"Coordinate the annual independent testing of the AML program. Engage qualified third party or internal audit. Retain testing report and address all findings.","category":"AML"},
        ...
        "POL-002": [
            {"id":"PT-002-1","title":"Supervision Policy Annual Review","owner":"admin","role_title":"Chief Compliance Officer","frequency":"Annually","description":"Review the firm's supervision policy for completeness and accuracy. Confirm all lines of business and product types are covered. Update as needed and obtain approval.","category":"Supervisory"},
            ...
            {"id":"PT-002-3","title":"Reg BI Best Interest Review","owner":"admin","role_title":"Chief Compliance Officer","frequency":"Quarterly","description":"Review a sample of retail recommendations to confirm they meet Reg BI best interest standards. Document findings and any remediation required.","category":"Regulatory"},
        ...
        "POL-003": [
            {"id":"PT-003-1","title":"Information Barrier Acknowledgment Collection","owner":"analyst","role_title":"Compliance Analyst","frequency":"Annually","description":"Collect signed information barrier acknowledgment forms from all employees in covered functions. Retain executed forms in the compliance records system.","category":"Trading"},
        ],
        "POL-004": [
            {"id":"PT-004-1","title":"Reg BI Conflict of Interest Review","owner":"admin","role_title":"Chief Compliance Officer","frequency":"Quarterly","description":"Review all identified conflicts of interest under Reg BI. Confirm disclosures are current. Evaluate whether any new conflicts have arisen and update Form CRS if required.","category":"Regulatory"},
    ...
    return {
        ...
        "tasks": [
            {"id":"T-001","title":"AML Transaction Monitoring Review","assignee":"analyst","due":(now+timedelta(days=3)).strftime("%Y-%m-%d"),"status":"Open","priority":"High","category":"AML",
             ...
             "evidence":[],"audit":[{"ts":ts(5),"user":"admin","action":"Created task T-001"}],"rationale":"","delegated_to":"","delegated_memo":"","source_wsp":"WSP-001","recurrence":"Quarterly"},
            {"id":"T-002","title":"Q3 Supervisory Checklist – Options","assignee":"supervisor","due":(now-timedelta(days=2)).strftime("%Y-%m-%d"),"status":"Overdue","priority":"High","category":"Supervisory",
             ...
             "evidence":[],"audit":[{"ts":ts(10),"user":"admin","action":"Created task T-002"}],"rationale":"","delegated_to":"","delegated_memo":"","source_wsp":"WSP-002","recurrence":"Quarterly"},
            {"id":"T-003","title":"Best Execution Policy Review","assignee":"admin","due":(now+timedelta(days=10)).strftime("%Y-%m-%d"),"status":"Open","priority":"Medium","category":"Trading",
            ...
            {"id":"T-004","title":"FINRA Rule 3110 Supervision Update","assignee":"analyst","due":(now+timedelta(days=1)).strftime("%Y-%m-%d"),"status":"Pending Approval","priority":"High","category":"Regulatory",
             ...
             "evidence":[{"name":"supervision_policy_v2.pdf","uploaded_by":"analyst","ts":ts(1),"hash":"abc123","immutable":True,"notes":"Clean copy of revised supervisory procedures v2.0 incorporating Rule 3110 amendments."}],"audit":[{"ts":ts(7),"user":"admin","action":"Created task T-004"},{"ts":ts(1),"user":"analyst","action":"Evidence uploaded: supervision_policy_v2.pdf"}],"rationale":"","delegated_to":"","delegated_memo":"","source_wsp":"WSP-002","recurrence":""},
            {"id":"T-005","title":"Annual Cybersecurity Training Completion","assignee":"analyst","due":(now-timedelta(days=5)).strftime("%Y-%m-%d"),"status":"Closed","priority":"Low","category":"InfoSec",
             ...
             "evidence":[],"audit":[{"ts":ts(15),"user":"admin","action":"Created task T-005"},{"ts":ts(5),"user":"admin","action":"Closed – training certificates verified"}],"rationale":"All staff completed training by deadline.","delegated_to":"","delegated_memo":"","source_wsp":"","recurrence":"Annually"},
        ],
        "exam_requests": [
        ...
        "policies": [
            {
                ...
                "owner":"admin","approved_by":"supervisor","approved_ts":ts(20),
                ...
                "versions":[{"ver":"2.0","ts":ts(90),"author":"admin","note":"Annual review — no material changes"},{"ver":"2.1","ts":ts(20),"author":"admin","note":"Updated FinCEN beneficial ownership rule references"}],
                "files":[],"reviewer":"","review_sent_ts":"",
                "audit":[
                    {"ts":ts(90),"user":"admin","action":"POL-001 v2.0 annual review completed"},
                    {"ts":ts(20),"user":"admin","action":"v2.1 uploaded — updated FinCEN references"},
                    {"ts":ts(20),"user":"admin","action":"Sent for review to supervisor"},
            ...
            },
            {
                ...
                "audit":[
                    ...
                    {"ts":ts(35),"user":"admin","action":"POL-002 v3.0 approved"},
                ],
            },
            {
                ...
                "related_tests":[],
                "owner":"analyst","approved_by":"admin","approved_ts":ts(50),
                ...
                "versions":[{"ver":"1.1","ts":ts(180),"author":"analyst","note":"Initial approved version"},{"ver":"1.2","ts":ts(50),"author":"analyst","note":"Added electronic communication surveillance requirements"}],
                "files":[],"reviewer":"","review_sent_ts":"",
                "audit":[
                    {"ts":ts(180),"user":"analyst","action":"POL-003 v1.1 created"},
                    {"ts":ts(50),"user":"analyst","action":"v1.2 uploaded — added e-comm surveillance section"},
                    {"ts":ts(50),"user":"analyst","action":"Sent for review to admin"},
                    {"ts":ts(50),"user":"admin","action":"POL-003 v1.2 approved"},
                ],
            },
            {
                ...
                "versions":[{"ver":"1.0","ts":ts(10),"author":"analyst","note":"Initial draft for review"}],
                "files":[
                    {"name":"POL-004_v1.0_draft.docx","type":"Clean","uploaded_by":"analyst","ts":ts(10),"notes":"Initial clean draft for CCO review and approval."},
                ],
                "reviewer":"admin","review_sent_ts":ts(9),
                "audit":[
                    {"ts":ts(10),"user":"analyst","action":"POL-004 v1.0 draft created"},
                    {"ts":ts(10),"user":"analyst","action":"Clean copy uploaded: POL-004_v1.0_draft.docx"},
                    {"ts":ts(9),"user":"analyst","action":"Sent for review to Jordan Reed (admin) — initial Reg BI policy draft, please review scope and conflict of interest section."},
                ],
            },
        ],
        "audit_trail": [
        ...
        "supervisory_checklists": [
            # POL-001 — AML Policy — 2 owners: admin (Jordan), supervisor (Morgan)
            {
                "id":"CL-001","source_wsp":"POL-001","wsp_title":"Anti-Money Laundering Policy",
                "wsp_version":"2.1","line_of_business":"Operations",
                "assigned_to":"admin","owner_name":"Jordan Reed","owner_title":"Chief Compliance Officer",
                "due":(now+timedelta(days=30)).strftime("%Y-%m-%d"),"recurrence":"Annually",
                "created_ts":ts(20),"created_by":"system","status":"Active",
                "items":[
                    {"wt_id":"PT-001-1","title":"Annual AML Policy Certification","description":"Review and certify the firm's AML Policy is current, reflects applicable FinCEN guidance, and has been distributed to all required personnel.","frequency":"Annually","category":"AML","status":"Complete","evidence":[{"name":"AML_Policy_Certification_2025.pdf","uploaded_by":"admin","ts":ts(5),"hash":"abc001","notes":"Signed certification — AML Policy v2.1 reviewed and certified as current."}],"completed_ts":ts(5),"checked_off":True,"supervisor_review":True,"supervisor_reviewed_by":"supervisor","supervisor_reviewed_ts":ts(3),"supervisor_notes":"Certification reviewed and signed off.","audit":[{"ts":ts(20),"user":"system","action":"Generated from POL-001 v2.1"},{"ts":ts(5),"user":"admin","action":"Evidence uploaded: AML_Policy_Certification_2025.pdf"},{"ts":ts(5),"user":"admin","action":"Marked complete"},{"ts":ts(3),"user":"supervisor","action":"Supervisor sign-off"}]},
                    {"wt_id":"PT-001-2","title":"AML Program Independent Testing Coordination","description":"Coordinate the annual independent testing of the AML program. Engage qualified third party or internal audit. Retain testing report and address all findings.","frequency":"Annually","category":"AML","status":"Complete","evidence":[{"name":"CT003_AML_Independent_Testing_Report_2025.pdf","uploaded_by":"admin","ts":ts(21),"hash":"abc002","notes":"Final testing report from independent AML review — CT-003."}],"completed_ts":ts(21),"checked_off":True,"supervisor_review":True,"supervisor_reviewed_by":"supervisor","supervisor_reviewed_ts":ts(20),"supervisor_notes":"Testing report reviewed. One finding noted — remediation tracked in CT-003.","audit":[{"ts":ts(20),"user":"system","action":"Generated from POL-001 v2.1"},{"ts":ts(21),"user":"admin","action":"Evidence uploaded: CT003_AML_Independent_Testing_Report_2025.pdf"},{"ts":ts(21),"user":"admin","action":"Marked complete"},{"ts":ts(20),"user":"supervisor","action":"Supervisor sign-off"}]},
                ],
                "audit":[{"ts":ts(20),"user":"system","action":"Auto-generated from POL-001 v2.1"}],
            },
            {
                "id":"CL-002","source_wsp":"POL-001","wsp_title":"Anti-Money Laundering Policy",
                "wsp_version":"2.1","line_of_business":"Operations",
                "assigned_to":"supervisor","owner_name":"Morgan Ellis","owner_title":"VP, Head of Supervision",
                "due":(now+timedelta(days=15)).strftime("%Y-%m-%d"),"recurrence":"Semi-Annually",
                "created_ts":ts(20),"created_by":"system","status":"Active",
                "items":[
                    {"wt_id":"PT-001-3","title":"SAR Filing Review & Submission","description":"Review all potential SAR candidates flagged by monitoring. Approve, escalate, or document no-file decisions within regulatory deadlines.","frequency":"As needed / 30-day SAR deadline","category":"AML","status":"Pending","evidence":[],"completed_ts":"","checked_off":False,"supervisor_review":False,"supervisor_reviewed_by":"","supervisor_reviewed_ts":"","supervisor_notes":"","audit":[{"ts":ts(20),"user":"system","action":"Generated from POL-001 v2.1"}]},
                    {"wt_id":"PT-001-4","title":"Customer Identification Program (CIP) Audit","description":"Audit a sample of new account openings to confirm CIP procedures were followed. Document findings and remediate exceptions.","frequency":"Semi-Annually","category":"AML","status":"Pending","evidence":[],"completed_ts":"","checked_off":False,"supervisor_review":False,"supervisor_reviewed_by":"","supervisor_reviewed_ts":"","supervisor_notes":"","audit":[{"ts":ts(20),"user":"system","action":"Generated from POL-001 v2.1"}]},
                ],
                "audit":[{"ts":ts(20),"user":"system","action":"Auto-generated from POL-001 v2.1"}],
            },
            # POL-002 — Supervision Policy — 2 owners: admin (Jordan), supervisor (Morgan)
            {
                "id":"CL-003","source_wsp":"POL-002","wsp_title":"Supervision and Supervisory Controls Policy",
                "wsp_version":"3.0","line_of_business":"Wealth Management",
                "assigned_to":"admin","owner_name":"Jordan Reed","owner_title":"Chief Compliance Officer",
                "due":(now+timedelta(days=14)).strftime("%Y-%m-%d"),"recurrence":"Annually",
                "created_ts":ts(35),"created_by":"system","status":"Active",
                "items":[
                    {"wt_id":"PT-002-1","title":"Supervision Policy Annual Review","description":"Review the firm's supervision policy for completeness and accuracy. Confirm all lines of business and product types are covered. Update as needed and obtain approval.","frequency":"Annually","category":"Supervisory","status":"Pending","evidence":[],"completed_ts":"","checked_off":False,"supervisor_review":False,"supervisor_reviewed_by":"","supervisor_reviewed_ts":"","supervisor_notes":"","audit":[{"ts":ts(35),"user":"system","action":"Generated from POL-002 v3.0"}]},
                    {"wt_id":"PT-002-3","title":"Reg BI Best Interest Review","description":"Review a sample of retail recommendations to confirm they meet Reg BI best interest standards. Document findings and any remediation.","frequency":"Quarterly","category":"Regulatory","status":"Pending","evidence":[],"completed_ts":"","checked_off":False,"supervisor_review":False,"supervisor_reviewed_by":"","supervisor_reviewed_ts":"","supervisor_notes":"","audit":[{"ts":ts(35),"user":"system","action":"Generated from POL-002 v3.0"}]},
                ],
                "audit":[{"ts":ts(35),"user":"system","action":"Auto-generated from POL-002 v3.0"}],
            },
            {
                "id":"CL-004","source_wsp":"POL-002","wsp_title":"Supervision and Supervisory Controls Policy",
                "wsp_version":"3.0","line_of_business":"Wealth Management",
                "assigned_to":"supervisor","owner_name":"Morgan Ellis","owner_title":"VP, Head of Supervision",
                "due":(now+timedelta(days=7)).strftime("%Y-%m-%d"),"recurrence":"Quarterly",
                "created_ts":ts(35),"created_by":"system","status":"Active",
                "items":[
                    {"wt_id":"PT-002-2","title":"Quarterly Supervisory Controls Report","description":"Compile the quarterly supervisory controls report summarizing review activity, exceptions identified, and remediation status across all business lines.","frequency":"Quarterly","category":"Supervisory","status":"Awaiting Review","evidence":[{"name":"Q1_2026_Supervisory_Controls_Report.pdf","uploaded_by":"supervisor","ts":ts(2),"hash":"cde003","notes":"Q1 2026 SCP report covering trade review, complaint handling, and Reg BI sampling."}],"completed_ts":ts(2),"checked_off":True,"supervisor_review":False,"supervisor_reviewed_by":"","supervisor_reviewed_ts":"","supervisor_notes":"","audit":[{"ts":ts(35),"user":"system","action":"Generated from POL-002 v3.0"},{"ts":ts(2),"user":"supervisor","action":"Evidence uploaded: Q1_2026_Supervisory_Controls_Report.pdf"},{"ts":ts(2),"user":"supervisor","action":"Marked complete — sent for supervisor review"}]},
                ],
                "audit":[{"ts":ts(35),"user":"system","action":"Auto-generated from POL-002 v3.0"}],
            },
            # POL-003 — Info Barriers — 1 owner: analyst (Casey)
            {
                "id":"CL-005","source_wsp":"POL-003","wsp_title":"Information Barriers and Insider Trading Prevention Policy",
                "wsp_version":"1.2","line_of_business":"Investment Banking",
                "assigned_to":"analyst","owner_name":"Casey Park","owner_title":"Compliance Analyst",
                "due":(now+timedelta(days=60)).strftime("%Y-%m-%d"),"recurrence":"Annually",
                "created_ts":ts(50),"created_by":"system","status":"Active",
                "items":[
                    {"wt_id":"PT-003-1","title":"Information Barrier Acknowledgment Collection","description":"Collect signed information barrier acknowledgment forms from all employees in covered functions. Retain executed forms in the compliance records system.","frequency":"Annually","category":"Trading","status":"Pending","evidence":[],"completed_ts":"","checked_off":False,"supervisor_review":False,"supervisor_reviewed_by":"","supervisor_reviewed_ts":"","supervisor_notes":"","audit":[{"ts":ts(50),"user":"system","action":"Generated from POL-003 v1.2"}]},
                ],
                "audit":[{"ts":ts(50),"user":"system","action":"Auto-generated from POL-003 v1.2"}],
            },
        ],
        ...
        "compliance_tests": [
            {
                ...
                "procedures":[
                    ...
                    {"id":"P-003","step":3,
                     ...
                     "rem_assign":"admin","rem_due":(now+timedelta(days=21)).strftime("%Y-%m-%d"),
            ...
            },
            {
                ...
                "exceptions":[],
                "conclusion":"",
                "audit":[{"ts":ts(3),"user":"admin","action":"Test CT-002 created"}],
            },
            {
                "id":"CT-003",
                ...
                "procedures":[
                    {"id":"P-001","step":1,"title":"CIP Completeness Review",
                     ...
                     "rem_assign":"analyst","rem_due":(now+timedelta(days=10)).strftime("%Y-%m-%d"),
                     "tested_by":"admin","tested_ts":ts(25)},
                    {"id":"P-002","step":2,"title":"Transaction Monitoring Alert Review",
                     ...
                     "result":"Pass","exception":False,"has_remediation_item":False,
                     "tested_by":"admin","tested_ts":ts(22)},
                    {"id":"P-003","step":3,"title":"AML Training Completion",
                     ...
                     "result":"Pass","exception":False,"has_remediation_item":False,
                     "tested_by":"admin","tested_ts":ts(21)},
                ],
                "exceptions":[
                    {"id":"EX-002","test_id":"CT-003","procedure_id":"P-001","description":"2 new accounts missing FinCEN beneficial ownership certifications in CIP files.","root_cause":"Onboarding checklist did not include mandatory beneficial ownership step for corporate accounts opened via digital channel.","remediation_plan":"Obtain outstanding certifications and update onboarding workflow to require beneficial ownership cert before account activation.","assigned_to":"analyst","due_date":(now+timedelta(days=10)).strftime("%Y-%m-%d"),"status":"Open","created":ts(25)},
                ],
                ...
                "audit":[
                    {"ts":ts(45),"user":"admin","action":"Test CT-003 created"},
                    {"ts":ts(25),"user":"admin","action":"Procedure P-001 completed: Exception — 2 CIP gaps"},
                    {"ts":ts(22),"user":"admin","action":"Procedure P-002 completed: Pass"},
                    {"ts":ts(21),"user":"admin","action":"Procedure P-003 completed: Pass"},
                    {"ts":ts(20),"user":"admin","action":"Test submitted for review"},
            ...
            },
            {
                ...
                "conclusion":"",
                "audit":[
                    {"ts":ts(7),"user":"admin","action":"Test CT-004 created"},
...
# ── WSP REPOSITORY ─────────────────────────────────────────────────────────────
LINES_OF_BUSINESS = ["Wealth Management","Fixed Income","Investment Banking","Operations","Equity","Research","Technology","Compliance","All Firm"]

def _auto_parse_wsp(wsp):
    """Generates supervisory checklists from POLICIES only (POL- prefix).
    WSPs are reference procedures — checklists are always derived from the
    corresponding policy, never directly from a WSP, to avoid duplicates.
    One checklist is created per unique procedure owner."""
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
    doc_id  = wsp["id"]

    # Only generate from policies — WSPs do not create independent checklists
    ...
    # Remove any existing checklists for this policy (version update / re-generation)
    D["supervisory_checklists"] = [c for c in D.get("supervisory_checklists",[])
                                   if c.get("source_wsp") != doc_id]

    # Group tasks by owner — one checklist per owner per policy
    owners = {}
    for wt in task_list:
        o = wt["owner"]
    ...
    for owner_key, tasks in owners.items():
        ...
        for wt in tasks:
            cl_items.append({
                ...
                "supervisor_review":False,"supervisor_reviewed_by":"","supervisor_reviewed_ts":"",
                "supervisor_notes":"",
                "audit":[{"ts":now_str,"user":"system","action":f"Checklist item generated from {doc_id} v{wsp['version']} on approval"}],
            })
        D["supervisory_checklists"].append({
            "id":cl_id,
            "source_wsp":doc_id,          # always the POL- id
            "wsp_title":wsp["title"],
            "wsp_version":wsp["version"],
            "line_of_business":wsp.get("line_of_business",""),
            ...
            "recurrence":"Ongoing","created_ts":now_str,
            "created_by":"system","status":"Active","items":cl_items,
            "audit":[{"ts":now_str,"user":"system","action":f"Auto-generated from {doc_id} v{wsp['version']}"}],
        })
    return len(owners)
...
def page_wsp():
    ...
    def _render_library(docs, tasks_key, id_prefix):
        ...
        all_owners= sorted(set(D["users"].get(w["owner"],{}).get("name",w["owner"]) for w in approved))
        ...
        for grp_label, grp_docs in groups:
            ...
            for i, doc in enumerate(grp_docs):
                ...
                if is_expanded:
                    ...
                    # Checklists only generated from policies (POL-), never WSPs
                    if doc["id"].startswith("POL-"):
                        checklists = [c for c in D.get("supervisory_checklists",[]) if c.get("source_wsp")==doc["id"]]
                    else:
                        checklists = []
                    rel_test_ids = doc.get("related_tests",[])
                    rel_tests    = [t for t in D.get("compliance_tests",[]) if t["id"] in rel_test_ids]
                    # Evidence = only files uploaded to checklist items belonging to this policy
                    ev_count     = sum(len(item.get("evidence",[])) for c in checklists for item in c.get("items",[]))
                    ...
                    # ── Auto-generate checklists on expand if not yet done ─────
                    existing_cls = [c for c in D.get("supervisory_checklists",[]) if c.get("source_wsp")==doc["id"]]
                    if not existing_cls and task_list:
                        n = _auto_parse_wsp(doc)
                        add_audit("Policies & WSPs", f"{doc['id']}: {n} checklist(s) auto-generated on expand")
                        existing_cls = [c for c in D.get("supervisory_checklists",[]) if c.get("source_wsp")==doc["id"]]

                    st.markdown("<hr style='margin:.5rem 0'/>", unsafe_allow_html=True)
                    if existing_cls:
                        owners_str = ", ".join(sorted(set(c["owner_name"] for c in existing_cls)))
                        st.markdown(f"<div class='gc-alert gc-alert-ok' style='font-size:11px'>✓ {len(existing_cls)} checklist(s) active — assigned to: {owners_str}</div>", unsafe_allow_html=True)
    ...
    def _render_drafts(docs, id_prefix, tasks_key):
        ...
        for doc in drafts:
            ...
            if toggle(f"draft_{id_prefix}_{doc['id']}", f"{doc['id']} — {doc['title']}  ·  v{doc['version']}  ·  {doc.get('line_of_business','')}  ·  {rs_lbl}"):
                ...
                # Reviewer actions
                is_reviewer = (st.session_state.current_user == doc.get("reviewer") or current_role()=="Compliance Officer")
...
# ── SUPERVISORY CHECKLISTS ─────────────────────────────────────────────────────
def page_checklists():
    st.markdown("<div class='gc-header'><h1>SUPERVISORY CHECKLISTS</h1><p>Role-based checklists auto-generated from approved policies — evidence upload, completion tracking, and supervisor sign-off</p></div>", unsafe_allow_html=True)
    checklists = D.get("supervisory_checklists",[])
    if not checklists:
        st.markdown("<div class='gc-alert gc-alert-warn'>No checklists generated yet. Checklists are automatically created when you expand any approved policy or WSP in the <b>Policies &amp; WSPs</b> section, and when a new version is approved.</div>", unsafe_allow_html=True)
        return

    # Back-compat: ensure supervisor review fields exist on checklist items
    for cl in checklists:
        if "wsp_version"      not in cl: cl["wsp_version"]      = ""
        if "line_of_business" not in cl: cl["line_of_business"] = ""
    ...
    # Filter options
    fc1, fc2, fc3 = st.columns(3)
    all_lobs = sorted(set(cl.get("line_of_business","") for cl in checklists if cl.get("line_of_business")))
    f_lob    = fc1.selectbox("Line of Business", ["All"] + all_lobs)
    f_owner  = fc2.selectbox("Filter by Owner", ["All"]+list({cl["assigned_to"] for cl in checklists}), format_func=lambda x: "All" if x=="All" else D["users"].get(x,{}).get("name",x))
    f_wsp    = fc3.selectbox("Filter by Policy", ["All"]+list({cl["source_wsp"] for cl in checklists}))
    if f_lob   != "All": checklists = [cl for cl in checklists if cl.get("line_of_business") == f_lob]
    ...
    for cl in checklists:
        ...
        pending_review = len([i for i in cl["items"] if i["checked_off"] and not i.get("supervisor_review")])
        lob_tag = f" · {cl['line_of_business']}" if cl.get("line_of_business") else ""
        ver_tag = f" v{cl['wsp_version']}" if cl.get("wsp_version") else ""
        s_badge = badge("Complete","badge-closed") if pct==100 else badge("Active","badge-open")

        if toggle(f"cl_{cl['id']}", f"{cl['id']} | {cl['owner_name']} | {cl['wsp_title']}{ver_tag}{lob_tag} ({done}/{total} done)"):
            st.markdown(f"""
            <div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:.8rem;flex-wrap:wrap;gap:.5rem'>
              <div>
                ...
                <div style='font-size:12px;color:#6b7280'>Policy: {cl['source_wsp']}{ver_tag} · {cl.get('line_of_business','')} · Due: {cl['due']} · {cl['recurrence']}</div>
              </div>
              <div style='display:flex;gap:.4rem;flex-wrap:wrap'>{s_badge}&nbsp;{badge(f"{pct}% done","badge-pending" if pct<100 else "badge-closed")}{("&nbsp;"+badge(f"{pending_review} awaiting supervisor review","badge-high")) if pending_review else ""}</div>
            </div>
            """, unsafe_allow_html=True)
            st.progress(pct/100)
            st.markdown("<hr/>", unsafe_allow_html=True)
            ...
            for idx, item in enumerate(cl["items"]):
                ...
                is_supervisor = current_role() in ["Compliance Officer","Supervisor"]
                ...
                if sup_done:
                    status_icon = "✅"; status_color = "#166534"; status_bg = "#f0fdf4"; border_col = "#22c55e"
                    status_label = "Signed Off"
                elif item["checked_off"]:
                    status_icon = "⏳"; status_color = "#92400e"; status_bg = "#fffbeb"; border_col = "#f59e0b"
                    status_label = "Awaiting Review"
                else:
                    status_icon = "⬜"; status_color = "#1e40af"; status_bg = "#eff6ff"; border_col = "#3b82f6"
                    status_label = "Pending"

                ev_str = f"<span style='color:#2563eb;font-weight:700'>{ev_ct} file(s)</span>" if ev_ct else "<span style='color:#9ca3af'>no evidence yet</span>"

                # Header row — always visible
                st.markdown(f"""
                <div style='background:{status_bg};border:1px solid #e5e7eb;border-left:3px solid {border_col};
                     border-radius:6px 6px 0 0;padding:.45rem .8rem;margin-top:4px'>
                  <div style='display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.3rem'>
                    <div>
                      <span style='font-size:11px;font-weight:800;color:#111827'>{status_icon} &nbsp;{item['title']}</span>
                      <span style='font-size:10px;color:#6b7280;margin-left:.6rem'>Policy: <b>{cl['source_wsp']}</b> · {cl['wsp_title']}</span>
                    </div>
                    <div style='display:flex;gap:.4rem;align-items:center;flex-wrap:wrap'>
                      <span style='background:#f1f5f9;color:#374151;font-size:9px;font-weight:700;padding:1px 6px;border-radius:8px'>🔁 {item['frequency']}</span>
                      <span class='gc-badge {cat_cls}'>{item['category']}</span>
                      <span style='background:{status_color}18;color:{status_color};font-size:9px;font-weight:700;padding:1px 6px;border-radius:8px'>{status_label}</span>
                      <span style='font-size:9px;color:#6b7280'>📎 {ev_str}</span>
                    </div>
                  </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                detail_key = f"cl_detail_{cl['id']}_{idx}"
                detail_open = st.session_state.get(detail_key, False)
                if st.button("▼ Details" if not detail_open else "▲ Hide", key=f"cl_tog_{cl['id']}_{idx}"):
                    st.session_state[detail_key] = not detail_open
                    st.rerun()

                if detail_open:
                    st.markdown(f"""
                    <div style='background:#ffffff;border:1px solid #e5e7eb;border-top:none;border-left:3px solid {border_col};
                         border-radius:0 0 6px 6px;padding:.6rem .8rem'>
                      <div style='font-size:9px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.07em;margin-bottom:.3rem'>WHAT TO DO</div>
                      <div style='font-size:12px;color:#374151;line-height:1.65;margin-bottom:.6rem'>{item['description']}</div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    if item["evidence"]:
                        for ev in item["evidence"]:
                            ev_note_html = f" · <i style='color:#6b7280'>{ev['notes']}</i>" if ev.get("notes") else ""
                            st.markdown(f"<div class='gc-tl' style='font-size:11px;margin-left:.5rem'>📎 <b>{ev['name']}</b> · {ev['uploaded_by']} · {ev['ts'][:16]}{ev_note_html}</div>", unsafe_allow_html=True)

                    # Controls
                    if is_mine and not item["checked_off"]:
                        up_col, done_col = st.columns([3, 1])
                        up = up_col.file_uploader(f"Upload evidence", key=f"cl_up_{cl['id']}_{idx}", label_visibility="collapsed")
                        cl_notes = up_col.text_input("Notes", placeholder="Describe the file or any exceptions…", key=f"cl_notes_{cl['id']}_{idx}", label_visibility="collapsed")
                        if up:
                            fh = hashlib.md5(up.name.encode()).hexdigest()[:8]
                            ...
                            now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
                            note_sfx = f" — {cl_notes.strip()}" if cl_notes.strip() else ""
                            item["audit"].append({"ts":now_str,"user":st.session_state.current_user,"action":f"Evidence uploaded: {up.name}{note_sfx}"})
                            cl["audit"].append({"ts":now_str,"user":st.session_state.current_user,"action":f"Evidence uploaded for '{item['title']}': {up.name}{note_sfx}"})
                            add_audit("Supervisory Checklists", f"{cl['id']}: evidence uploaded for '{item['title']}'")
                            st.success(f"✓ {up.name} attached"); st.rerun()
                        if done_col.button("✅ Mark Complete", key=f"cl_done_{cl['id']}_{idx}"):
                            now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
                            item["checked_off"] = True; item["completed_ts"] = now_str; item["status"] = "Complete"
                            item["audit"].append({"ts":now_str,"user":st.session_state.current_user,"action":"Marked complete — sent for supervisor review"})
                    ...
                    elif item["checked_off"] and not sup_done:
                        comp_user = item["audit"][-1]["user"] if item.get("audit") else "—"
                        st.markdown(f"<div style='font-size:11px;color:#92400e;background:#fffbeb;border-radius:4px;padding:.3rem .6rem;margin:.3rem 0'>⏳ Completed by <b>{comp_user}</b> on {item.get('completed_ts','')[:16]} — awaiting supervisor sign-off</div>", unsafe_allow_html=True)
                        if is_supervisor:
                            sv1, sv2 = st.columns([3, 1])
                            sv_notes = sv1.text_input("Supervisor notes", placeholder="Approval notes or follow-up…", key=f"sv_notes_{cl['id']}_{idx}", label_visibility="collapsed")
                            if sv2.button("✅ Sign Off", key=f"sv_done_{cl['id']}_{idx}"):
                                ...
                                item["supervisor_notes"]       = sv_notes.strip()
                                item["audit"].append({"ts":now_str,"user":st.session_state.current_user,"action":"Supervisor sign-off complete" + (f" — {sv_notes.strip()}" if sv_notes.strip() else "")})
                                cl["audit"].append({"ts":now_str,"user":st.session_state.current_user,"action":f"Supervisor reviewed '{item['title']}'"})
                                add_audit("Supervisory Checklists", f"{cl['id']}: supervisor sign-off for '{item['title']}'")
                                st.rerun()

                    elif item["checked_off"] and sup_done:
                        sv_name = D["users"].get(item.get("supervisor_reviewed_by",""),{}).get("name", item.get("supervisor_reviewed_by","—"))
                        st.markdown(f"<div style='font-size:11px;color:#166534;margin:.2rem 0'>✅ Complete · Signed off by <b>{sv_name}</b> · {item.get('supervisor_reviewed_ts','')[:16]}" + (f" · <i>{item['supervisor_notes']}</i>" if item.get('supervisor_notes') else "") + "</div>", unsafe_allow_html=True)
...
COMPLIANCE_CALENDAR_ITEMS = [
    # ── JANUARY ──
    {"id":"CAL-001","regulator":"FINRA","item":"Annual Renewal — Final Statement Discrepancy Report Due","category":"Registration","deadline":"2026-01-23","rule":"FINRA Renewal Program","assignee":"admin","notes":"Report any discrepancies found on the Final Renewal Statement to FINRA in writing."},
    {"id":"CAL-002","regulator":"FINRA","item":"Annual Renewal — Final Payment Due","category":"Registration","deadline":"2026-01-23","rule":"FINRA Renewal Program","assignee":"admin","notes":"Submit any outstanding balance from the Final Renewal Statement. FINRA will transfer from Flex-Funding Account if authorized."},
    {"id":"CAL-003","regulator":"FINRA","item":"FOCUS Report Part II/IIA — Monthly Filing (December 2025)","category":"Financial Reporting","deadline":"2026-01-26","rule":"SEA Rule 17a-5 / FINRA Rule 4524","assignee":"admin","notes":"Monthly FOCUS Report Part II or IIA due for December 2025 reporting period. File via eFOCUS through FINRA Gateway."},
    {"id":"CAL-004","regulator":"FINRA","item":"Supplemental Inventory Schedule (SIS) — Q4 2025 Filing","category":"Financial Reporting","deadline":"2026-01-28","rule":"FINRA Rule 4524","assignee":"admin","notes":"SIS filing due within 22 business days of December 31, 2025 quarter-end."},
    ...
    # ── FEBRUARY ──
    {"id":"CAL-006","regulator":"FINRA","item":"FOCUS Report Part II/IIA — Monthly Filing (January 2026)","category":"Financial Reporting","deadline":"2026-02-25","rule":"SEA Rule 17a-5 / FINRA Rule 4524","assignee":"admin","notes":"Monthly FOCUS Report for January 2026 reporting period."},
    {"id":"CAL-007","regulator":"FINRA","item":"Annual Report (Fiscal Year-End Dec 31, 2025) — Standard Deadline","category":"Financial Reporting","deadline":"2026-03-02","rule":"SEA Rule 17a-5(d)","assignee":"admin","notes":"Annual Report (audited financial statements) due 60 days after fiscal year-end for standard filers. File via FINRA Gateway; also file on EDGAR in PDF format per new SEC requirements."},
    {"id":"CAL-008","regulator":"SEC","item":"Annual Report on Form X-17A-5 Part III — EDGAR Filing","category":"Financial Reporting","deadline":"2026-03-02","rule":"SEA Rule 17a-5(d)","assignee":"admin","notes":"Broker-dealers must file Annual Report with SEC via EDGAR in PDF format in addition to FINRA Gateway filing."},
    # ── MARCH ──
    {"id":"CAL-009","regulator":"FINRA","item":"FOCUS Report Part II/IIA — Monthly Filing (February 2026)","category":"Financial Reporting","deadline":"2026-03-24","rule":"SEA Rule 17a-5 / FINRA Rule 4524","assignee":"admin","notes":"Monthly FOCUS Report for February 2026 reporting period."},
    {"id":"CAL-010","regulator":"FINRA","item":"Supplemental Inventory Schedule (SIS) — Q1 2026 Interim Filing","category":"Financial Reporting","deadline":"2026-03-27","rule":"FINRA Rule 4524","assignee":"admin","notes":"SIS filing due within 22 business days of end of Q4 2025 (December 31, 2025)."},
    {"id":"CAL-011","regulator":"FINRA","item":"Annual Report — Extended Deadline (Smaller B-Ds with 30-day extension)","category":"Financial Reporting","deadline":"2026-03-31","rule":"SEA Rule 17a-5(d) / SEC Extension Order","assignee":"admin","notes":"Firms qualifying for the SEC's 30-day extension order may file Annual Report by this date. Must have submitted notification to FINRA per Regulatory Notice 21-05."},
    ...
    {"id":"CAL-013","regulator":"SEC","item":"Annual CEO Certification — Reg BI Compliance (Form BD Update if needed)","category":"Reg BI / Form CRS","deadline":"2026-03-31","rule":"SEC Reg BI","assignee":"admin","notes":"Annual internal review of Reg BI compliance program. Document findings and remediation steps taken since last review."},
    # ── APRIL ──
    {"id":"CAL-014","regulator":"FINRA","item":"Supplemental Liquidity Schedule (SLS) — Q1 2026 Filing","category":"Financial Reporting","deadline":"2026-04-02","rule":"FINRA Rule 4524","assignee":"admin","notes":"SLS filing due within 2 business days of end of Q1 2026 for firms required to file."},
    {"id":"CAL-015","regulator":"FINRA","item":"Form Custody Filing — Q1 2026","category":"Financial Reporting","deadline":"2026-04-23","rule":"SEA Rule 17a-5(f) / FINRA Rule 4524","assignee":"admin","notes":"Form Custody due within 17 business days of end of Q1 2026 (March 31, 2026)."},
    {"id":"CAL-016","regulator":"FINRA","item":"FOCUS Report Part II/IIA — Monthly Filing (March 2026)","category":"Financial Reporting","deadline":"2026-04-23","rule":"SEA Rule 17a-5 / FINRA Rule 4524","assignee":"admin","notes":"Monthly FOCUS Report for March 2026 reporting period."},
    {"id":"CAL-017","regulator":"FINRA","item":"Form OBS (Derivatives & Off-Balance Sheet) — Q1 2026","category":"Financial Reporting","deadline":"2026-04-30","rule":"FINRA Rule 4524","assignee":"admin","notes":"OBS filing due within 22 business days of March 31, 2026 quarter-end for applicable firms."},
    ...
    # ── MAY ──
    {"id":"CAL-019","regulator":"FINRA","item":"FOCUS Report Part II/IIA — Monthly Filing (April 2026)","category":"Financial Reporting","deadline":"2026-05-26","rule":"SEA Rule 17a-5 / FINRA Rule 4524","assignee":"admin","notes":"Monthly FOCUS Report for April 2026 reporting period."},
    {"id":"CAL-020","regulator":"FINRA","item":"Annual AML Independent Testing — Complete by Mid-Year","category":"AML / BSA","deadline":"2026-05-31","rule":"FINRA Rule 3310 / BSA","assignee":"admin","notes":"Independent (internal audit or third party) testing of AML program must be completed. Testing must cover CIP, transaction monitoring, SAR filing, and employee training. Document results and remediation."},
    ...
    # ── JUNE ──
    {"id":"CAL-022","regulator":"FINRA","item":"FOCUS Report Part II/IIA — Monthly Filing (May 2026)","category":"Financial Reporting","deadline":"2026-06-24","rule":"SEA Rule 17a-5 / FINRA Rule 4524","assignee":"admin","notes":"Monthly FOCUS Report for May 2026 reporting period."},
    {"id":"CAL-023","regulator":"FINRA","item":"Supplemental Inventory Schedule (SIS) — Q2 2026 Filing","category":"Financial Reporting","deadline":"2026-06-30","rule":"FINRA Rule 4524","assignee":"admin","notes":"SIS filing due within 22 business days of June 30, 2026 quarter-end."},
    {"id":"CAL-024","regulator":"SEC","item":"Rule 15c3-3 Daily Reserve Formula Computation — Compliance Deadline","category":"Financial Responsibility","deadline":"2026-06-30","rule":"SEA Rule 15c3-3(e)(3)(i)(B)(1)","assignee":"admin","notes":"Firms with minimum fixed dollar net capital >= $250K as of Dec 31, 2025 must begin daily reserve formula computation by this date per amended Rule 15c3-3."},
    ...
    # ── JULY ──
    {"id":"CAL-026","regulator":"FINRA","item":"FOCUS Report Part II/IIA — Monthly Filing (June 2026)","category":"Financial Reporting","deadline":"2026-07-23","rule":"SEA Rule 17a-5 / FINRA Rule 4524","assignee":"admin","notes":"Monthly FOCUS Report for June 2026 reporting period."},
    {"id":"CAL-027","regulator":"FINRA","item":"Supplemental Liquidity Schedule (SLS) — Q2 2026 Filing","category":"Financial Reporting","deadline":"2026-07-02","rule":"FINRA Rule 4524","assignee":"admin","notes":"SLS filing due within 2 business days of end of Q2 2026 for applicable firms."},
    {"id":"CAL-028","regulator":"FINRA","item":"Form Custody Filing — Q2 2026","category":"Financial Reporting","deadline":"2026-07-23","rule":"SEA Rule 17a-5(f) / FINRA Rule 4524","assignee":"admin","notes":"Form Custody due within 17 business days of end of Q2 2026 (June 30, 2026)."},
    {"id":"CAL-029","regulator":"FINRA","item":"Form OBS (Derivatives & Off-Balance Sheet) — Q2 2026","category":"Financial Reporting","deadline":"2026-07-31","rule":"FINRA Rule 4524","assignee":"admin","notes":"OBS filing due within 22 business days of June 30, 2026 quarter-end."},
    ...
    # ── AUGUST ──
    {"id":"CAL-031","regulator":"FINRA","item":"FOCUS Report Part II/IIA — Monthly Filing (July 2026)","category":"Financial Reporting","deadline":"2026-08-25","rule":"SEA Rule 17a-5 / FINRA Rule 4524","assignee":"admin","notes":"Monthly FOCUS Report for July 2026 reporting period."},
    ...
    {"id":"CAL-033","regulator":"SEC","item":"Regulation S-P Cybersecurity Incident Response — Program Assessment","category":"Customer Privacy","deadline":"2026-08-31","rule":"SEC Regulation S-P (amended)","assignee":"admin","notes":"Assess cybersecurity incident response and notification procedures under amended Reg S-P. Ensure policies cover unauthorized access to customer financial information and 30-day customer notification requirement."},
    # ── SEPTEMBER ──
    {"id":"CAL-034","regulator":"FINRA","item":"FOCUS Report Part II/IIA — Monthly Filing (August 2026)","category":"Financial Reporting","deadline":"2026-09-24","rule":"SEA Rule 17a-5 / FINRA Rule 4524","assignee":"admin","notes":"Monthly FOCUS Report for August 2026 reporting period."},
    {"id":"CAL-035","regulator":"FINRA","item":"Supplemental Inventory Schedule (SIS) — Q3 2026 Filing","category":"Financial Reporting","deadline":"2026-09-30","rule":"FINRA Rule 4524","assignee":"admin","notes":"SIS filing due within 22 business days of September 30, 2026 quarter-end."},
    ...
    {"id":"CAL-037","regulator":"FINRA","item":"Business Continuity Plan (BCP) Annual Review and Update","category":"Operations","deadline":"2026-09-30","rule":"FINRA Rule 4370","assignee":"admin","notes":"Annual review of the firm's BCP. Update for any changes in personnel, systems, or business lines. Test emergency contact lists. Document review and any updates made."},
    # ── OCTOBER ──
    {"id":"CAL-038","regulator":"FINRA","item":"FOCUS Report Part II/IIA — Monthly Filing (September 2026)","category":"Financial Reporting","deadline":"2026-10-22","rule":"SEA Rule 17a-5 / FINRA Rule 4524","assignee":"admin","notes":"Monthly FOCUS Report for September 2026 reporting period."},
    {"id":"CAL-039","regulator":"FINRA","item":"Supplemental Liquidity Schedule (SLS) — Q3 2026 Filing","category":"Financial Reporting","deadline":"2026-10-02","rule":"FINRA Rule 4524","assignee":"admin","notes":"SLS filing due within 2 business days of end of Q3 2026 for applicable firms."},
    {"id":"CAL-040","regulator":"FINRA","item":"Form Custody Filing — Q3 2026","category":"Financial Reporting","deadline":"2026-10-22","rule":"SEA Rule 17a-5(f) / FINRA Rule 4524","assignee":"admin","notes":"Form Custody due within 17 business days of end of Q3 2026 (September 30, 2026)."},
    {"id":"CAL-041","regulator":"FINRA","item":"Form OBS (Derivatives & Off-Balance Sheet) — Q3 2026","category":"Financial Reporting","deadline":"2026-10-30","rule":"FINRA Rule 4524","assignee":"admin","notes":"OBS filing due within 22 business days of September 30, 2026 quarter-end."},
    {"id":"CAL-042","regulator":"FINRA","item":"Annual Renewal Program — Preliminary Statement Review","category":"Registration","deadline":"2026-10-31","rule":"FINRA Renewal Program","assignee":"admin","notes":"Review Preliminary Renewal Statement. Verify all registrations to be renewed and confirm budget for renewal fees. Begin ensuring sufficient funds in Renewal Account or Flex-Funding Account."},
    # ── NOVEMBER ──
    {"id":"CAL-043","regulator":"FINRA","item":"FOCUS Report Part II/IIA — Monthly Filing (October 2026)","category":"Financial Reporting","deadline":"2026-11-24","rule":"SEA Rule 17a-5 / FINRA Rule 4524","assignee":"admin","notes":"Monthly FOCUS Report for October 2026 reporting period."},
    {"id":"CAL-044","regulator":"FINRA","item":"Annual Renewal — Full Payment Due (Preliminary Statement)","category":"Registration","deadline":"2026-12-01","rule":"FINRA Renewal Program","assignee":"admin","notes":"Full payment of Preliminary Renewal Statement must be available in Renewal Account or Flex-Funding Account. Failure to pay may result in late fees and automatic termination of registrations."},
    {"id":"CAL-045","regulator":"FINRA","item":"Mass Transfer Moratorium Begins — Last Day for Mass Transfers","category":"Registration","deadline":"2026-11-27","rule":"FINRA Renewal Program","assignee":"admin","notes":"Last day to request a mass transfer (used for mergers, acquisitions, succession). Moratorium on mass transfer processing begins Dec 19 through Jan 1, 2027."},
    {"id":"CAL-046","regulator":"SEC","item":"Annual Compliance Program Review (SEC 206(4)-7 / FINRA 3130)","category":"Supervision","deadline":"2026-11-30","rule":"SEC Rule 206(4)-7 / FINRA Rule 3130","assignee":"admin","notes":"CEO must certify annual review of compliance policies and procedures. Written report to senior management required. Review must address any material compliance matters identified during the year."},
    ...
    # ── DECEMBER ──
    {"id":"CAL-048","regulator":"FINRA","item":"FOCUS Report Part II/IIA — Monthly Filing (November 2026)","category":"Financial Reporting","deadline":"2026-12-23","rule":"SEA Rule 17a-5 / FINRA Rule 4524","assignee":"admin","notes":"Monthly FOCUS Report for November 2026 reporting period."},
    {"id":"CAL-049","regulator":"FINRA","item":"Annual Renewal — CRD/IARD System Shutdown","category":"Registration","deadline":"2026-12-26","rule":"FINRA Renewal Program","assignee":"admin","notes":"CRD/IARD shuts down at 6 p.m. ET on Dec 26. Submit all registration filings before shutdown. Terminate any registrations not being renewed. Finalize all E-Bill renewal payments."},
    ...
    {"id":"CAL-051","regulator":"FINRA","item":"Supplemental Inventory Schedule (SIS) — Q4 2026 Filing Prep","category":"Financial Reporting","deadline":"2026-12-31","rule":"FINRA Rule 4524","assignee":"admin","notes":"Ensure all positions are accurately recorded as of December 31, 2026 for SIS filing due in late January 2027."},
    {"id":"CAL-052","regulator":"SEC","item":"Annual Net Capital Computation Review (Rule 15c3-1)","category":"Financial Responsibility","deadline":"2026-12-31","rule":"SEA Rule 15c3-1","assignee":"admin","notes":"Year-end review of net capital compliance. Ensure aggregate indebtedness/net capital ratio is within limits. Verify haircut calculations are current with regulatory requirements."},
]
...
def page_compliance_calendar():
    ...
    categories = sorted(set(i["category"] for i in COMPLIANCE_CALENDAR_ITEMS))
    ...
    # Apply filters
    items = COMPLIANCE_CALENDAR_ITEMS
    ...
    # ── Summary metrics ────────────────────────────────────────────────────────
    total_all   = len(COMPLIANCE_CALENDAR_ITEMS)
    done_all    = sum(1 for i in COMPLIANCE_CALENDAR_ITEMS if cal_state[i["id"]]["completed"])
    pending_all = total_all - done_all
    today_str   = datetime.now().strftime("%Y-%m-%d")
    overdue_all = sum(1 for i in COMPLIANCE_CALENDAR_ITEMS if not cal_state[i["id"]]["completed"] and i["deadline"] < today_str)
...
def page_compliance_testing():
    ...
    # ──────────────────────────────────────────────────────────────────────────
        ...
        with tt2:
            ...
            # Add custom test (admin/compliance only)
