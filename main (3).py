...
[data-testid="stMetricLabel"] { color: var(--text2) !important; font-size: 0.78rem !important; font-family: var(--mono) !important; }
...
.stDataFrame tbody td { color: var(--text) !important; font-size: 0.8rem !important; }
...
.badge-open    { background: rgba(74,158,255,0.18); color: #4a9eff; }
.badge-closed  { background: rgba(76,175,130,0.18); color: #4caf82; }
.badge-overdue { background: rgba(224,92,92,0.18);  color: #e05c5c; }
.badge-pending { background: rgba(240,165,0,0.18);  color: #f0a500; }
.badge-high    { background: rgba(224,92,92,0.18);  color: #e05c5c; }
.badge-med     { background: rgba(240,165,0,0.18);  color: #f0a500; }
.badge-low     { background: rgba(76,175,130,0.18); color: #4caf82; }
...
/* Header banner */
.gc-header {
  background: linear-gradient(135deg, var(--surface) 0%, #181d2a 100%);
  border: 1px solid var(--border);
...
.gc-header p { font-size: 0.82rem; color: var(--text2); margin: 0; }

/* Alert boxes */
.gc-alert { border-radius: 4px; padding: 0.7rem 1rem; margin: 0.5rem 0; font-size: 0.82rem; }
.gc-alert-warn { background: rgba(240,165,0,0.1); border-left: 3px solid var(--amber); color: var(--amber); }
.gc-alert-danger { background: rgba(224,92,92,0.1); border-left: 3px solid var(--danger); color: var(--danger); }
.gc-alert-ok { background: rgba(76,175,130,0.1); border-left: 3px solid var(--success); color: var(--success); }
...
.login-logo { font-family: var(--mono); font-size: 2rem; color: var(--amber); letter-spacing: 0.12em; text-align: center; margin-bottom: 0.3rem; }
.login-sub  { text-align: center; color: var(--text2); font-size: 0.8rem; margin-bottom: 2rem; font-family: var(--mono); letter-spacing: 0.06em; }
</style>
...
def sidebar():
    with st.sidebar:
        ...
        <div style='padding:0.8rem 0 1rem 0;border-bottom:1px solid var(--border);margin-bottom:1rem'>
          <div style='font-family:var(--mono);font-size:1.2rem;color:var(--amber);letter-spacing:0.1em'>⚖ GRAPHITE</div>
          <div style='font-size:0.65rem;color:var(--text2);font-family:var(--mono);letter-spacing:0.08em'>COMPLIANCE PLATFORM</div>
        </div>
        <div style='font-size:0.75rem;color:var(--text2);font-family:var(--mono);margin-bottom:0.3rem'>LOGGED IN AS</div>
        <div style='font-size:0.9rem;font-weight:600;color:var(--text);margin-bottom:0.2rem'>{current_name()}</div>
        <div style='font-size:0.72rem;color:var(--amber);font-family:var(--mono)'>{current_role()}</div>
        <hr/>
        """, unsafe_allow_html=True)
...
def page_dashboard():
    ...
    with col_l:
        st.markdown("<div class='gc-card-header' style='font-family:var(--mono);font-size:0.75rem;letter-spacing:0.1em;color:var(--amber);text-transform:uppercase;margin-bottom:0.8rem'>MY TASKS</div>", unsafe_allow_html=True)
    ...
    with col_r:
        st.markdown("<div class='gc-card-header' style='font-family:var(--mono);font-size:0.75rem;letter-spacing:0.1em;color:var(--amber);text-transform:uppercase;margin-bottom:0.8rem'>RECENT ACTIVITY</div>", unsafe_allow_html=True)
        for entry in reversed(D["audit_trail"][-8:]):
            st.markdown(f"""
            <div class='gc-timeline-item'>
              <span style='color:var(--amber);font-family:var(--mono);font-size:0.68rem'>{entry['ts']}</span>
        ...
        for t in tasks:
            ...
            with st.expander(f"[{t['id']}]  {t['title']}"):
                ...
                for entry in t["audit"]:
                    st.markdown(f"<div class='gc-timeline-item'><span style='color:var(--amber);font-family:var(--mono);font-size:0.68rem'>{entry['ts']}</span>  ·  <b>{entry['user']}</b>  ·  {entry['action']}</div>", unsafe_allow_html=True)
    ...
    with tab1:
        for wsp in D["wsps"]:
            ...
            with st.expander(f"[{wsp['id']}]  {wsp['title']}  ·  v{wsp['version']}"):
                ...
                st.markdown("<hr/>", unsafe_allow_html=True)
                st.markdown("<div class='gc-card-header' style='font-family:var(--mono);font-size:0.72rem;letter-spacing:0.1em;color:var(--amber);text-transform:uppercase;margin-bottom:0.6rem'>📋 Generate Supervisory Checklist from this WSP</div>", unsafe_allow_html=True)
                ...
                if st.button(f"✅ Create {len(suggested)} Checklist Task(s) from {wsp['id']}", key=f"gen_cl_{wsp['id']}"):
                    ...
                    add_audit("WSP Repository", f"Generated {len(created)} checklist tasks from {wsp['id']}: {', '.join(created)}")
                    st.success(f"✓ {len(created)} tasks created: {', '.join(created)} — visible in Tasks & Remediation")
                    st.rerun()
...
def page_evidence():
    ...
    for ev in ev_list:
        ...
        <div class='gc-card'>
          <div style='display:flex;justify-content:space-between;align-items:center'>
            <div>
              <span style='font-family:var(--mono);color:var(--amber);font-size:0.72rem'>{ev['id']}</span>  
    ...
    for node in D["org_chart"]:
        ...
        <div class='gc-card'>
          <div style='display:flex;justify-content:space-between;align-items:flex-start'>
            <div>
              <div style='font-family:var(--mono);color:var(--amber);font-size:0.8rem;letter-spacing:0.06em'>{node['role']}</div>
...
def page_audit():
    ...
    for entry in trail:
        st.markdown(f"""
        <div class='gc-timeline-item'>
          <span style='color:var(--amber);font-family:var(--mono);font-size:0.7rem'>{entry['ts']}</span>
