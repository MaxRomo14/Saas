        ...
        for grp_label, grp_docs in groups:
            ...
            for i, doc in enumerate(grp_docs):
                ...
                if is_expanded:
                    ...
                    btn_a, btn_b, _rest = st.columns([1, 1, 6])
                    if existing_cls:
                        if btn_a.button(f"🔄 Re-Parse — {doc['id']}", key=f"regen_{id_prefix}_{doc['id']}"):
                        ...
                        if btn_a.button(f"⚙️ Generate Checklists — {doc['id']}", key=f"gen_cl_{id_prefix}_{doc['id']}"):
                            n = _auto_parse_wsp(doc)
                            add_audit("Policies & WSPs", f"{doc['id']}: {n} checklist(s) generated")
                            st.success(f"✓ {n} checklist(s) generated."); st.rerun()
                    if btn_b.button("▲ Collapse", key=f"col_{id_prefix}_{doc['id']}"):
