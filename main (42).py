            ...
            # ── Jordan Reed (admin) — one checklist, all procedures he owns ──
            {
                ...
                "items": [
                    # From POL-001
                    {
                        ...
                        "evidence":[{"name":"AML_Policy_Certification_2025.pdf","uploaded_by":"admin","ts":ts(5),"hash":"abc001","notes":"Signed annual certification — AML Policy v2.1 reviewed and certified as current."}],
                        "comments":[],
                        "audit":[
                            {"ts":ts(35),"user":"system","action":"Item added from POL-001 v2.1"},
                            {"ts":ts(5),"user":"admin","action":"Evidence uploaded: AML_Policy_Certification_2025.pdf"},
                            {"ts":ts(5),"user":"admin","action":"Marked complete — routed to supervisor for sign-off"},
                    ...
                    },
                    {
                        ...
                        "evidence":[{"name":"CT003_AML_Independent_Testing_Report_2025.pdf","uploaded_by":"admin","ts":ts(21),"hash":"abc002","notes":"Final testing report from CT-003 independent AML program review."}],
                        "comments":[],
                        "audit":[
                            {"ts":ts(35),"user":"system","action":"Item added from POL-001 v2.1"},
                            {"ts":ts(21),"user":"admin","action":"Evidence uploaded: CT003_AML_Independent_Testing_Report_2025.pdf"},
                            {"ts":ts(21),"user":"admin","action":"Marked complete — routed to supervisor"},
        ...
        if toggle(f"cl_head_{cl['id']}",
            ...
            for idx, item in enumerate(items_sorted):
                ...
                if detail_open:
                    ...
                    is_sup_user  = current_role() in ["Compliance Officer","Supervisor"]
...
def _copilot_classify(q: str) -> str:
    q = q.lower()
    report_kw  = ["executive summary","create a report","board report","generate report","write a report","create an executive","download","memo","regulatory memo"]
    reg_kw     = ["finra","sec update","regulatory change","recent update","regulation update","which policies should be reviewed","rule update","regulatory changes"]
    test_kw    = ["3120","ct-","testing result","test result","summarize.*test","aml.*test","testing summary","compliance test","risk rating","exceptions"]
    ...
    nav_kw     = ["where","find","locate","show me the.*version","most recent version","evidence for","which wsps","which policies"]

    if any(k in q for k in report_kw):  return "generate_report"
    if any(k in q for k in reg_kw):     return "regulatory_analysis"
    if any(k in q for k in test_kw):    return "summarize_testing"
    if any(k in q for k in data_kw):    return "query_platform_data"
    if any(k in q for k in nav_kw):     return "search_platform_records"
...
def _handle_regulatory(q, raw):
    ...
    # Filter rules relevant to query
    pool = rules
    if "finra" in q:
        pool = [r for r in rules if r["source"]=="FINRA"]
    if "sec" in q and "finra" not in q:
        pool = [r for r in rules if r["source"]=="SEC"]
    if not pool:
        pool = rules

    html  = f"<div style='background:#fff;border:1px solid #d1d5db;border-radius:10px;padding:20px 24px'>"
    html += f"<div style='border-bottom:2px solid #1e3a5f;padding-bottom:12px;margin-bottom:16px'>"
    html += f"<div style='font-size:10px;font-weight:700;letter-spacing:.1em;color:#6b7280;text-transform:uppercase'>Regulatory Update Memo</div>"
    html += f"<div style='font-size:18px;font-weight:800;color:#1e3a5f;margin:4px 0'>Regulatory Change Analysis</div>"
    html += f"<div style='font-size:12px;color:#6b7280'>{firm.get('name','Apex Financial Services LLC')} · Dual Registrant (RIA / BD) · {today_str}</div>"
    html += "</div>"

    html += "<div style='font-size:13px;font-weight:700;color:#111;margin-bottom:6px'>Firm Profile</div>"
    html += f"<p style='font-size:12px;color:#374151;line-height:1.6'>This analysis applies to <b>{firm.get('name','Apex Financial Services LLC')}</b>, a dual registrant operating as a Registered Investment Adviser (SEC) and Broker-Dealer (FINRA). Active business lines include: <b>{', '.join(FIRM_CONTEXT['business_lines'])}</b>. The following regulatory rules in the firm's Rule Inventory have been identified as potentially requiring review.</p>"

    html += f"<div style='font-size:13px;font-weight:700;color:#111;margin:14px 0 8px'>Updates Identified ({len(pool)} rules)</div>"

    for r in pool:
        # Determine affected policies
        affected_pol = []
        for p in policies:
            regs_lower = [reg.lower() for reg in p.get("regulations", [])]
            if r["source"].lower() in regs_lower or r["title"].lower()[:20] in " ".join(regs_lower) or r["id"] in str(p.get("related_tests","")):
                affected_pol.append(p)
        for w in r.get("wsp_links", []):
            for wsp in wsps:
                if wsp["id"] == w["wsp_id"] and wsp not in affected_pol:
                    affected_pol.append(wsp)

        # Map rule to business lines
        bl_map = {
        ...
        affected_bls = bl_map.get(r["category"], FIRM_CONTEXT["business_lines"])
        ai_flag = r.get("ai_flagged", False)

        html += f"<div style='border:1px solid {'#fca5a5' if ai_flag else '#e5e7eb'};border-radius:8px;padding:14px 16px;margin-bottom:12px;{'background:#fff7f7' if ai_flag else ''}'>"
        html += f"<div style='display:flex;align-items:center;gap:10px;margin-bottom:8px'>"
        html += f"<span style='background:#dbeafe;color:#1e40af;border-radius:4px;padding:2px 8px;font-size:11px;font-weight:700'>{r['source']}</span>"
        html += f"<span style='font-weight:700;font-size:13px;color:#111'>{r['title']}</span>"
        if ai_flag: html += f"<span style='background:#fef2f2;color:#dc2626;border-radius:4px;padding:2px 8px;font-size:10px;font-weight:700;margin-left:auto'>⚠ FLAGGED</span>"
        html += "</div>"
        html += f"<div style='font-size:12px;color:#374151;margin-bottom:8px;line-height:1.6'><b>Summary:</b> {r['description']}</div>"
        html += f"<div style='font-size:12px;color:#374151;margin-bottom:8px'><b>Why it may matter to this firm:</b> As a dual registrant with active {', '.join(affected_bls[:2])} operations, this firm is likely subject to requirements under this rule. Review is recommended to confirm current procedures remain compliant.</div>"
        html += f"<div style='font-size:12px;margin-bottom:6px'><b>Potentially Impacted Business Lines:</b> {', '.join(affected_bls)}</div>"
        if affected_pol:
            pol_refs = ", ".join([f"<span style='color:#2563eb'>{p.get('id','WSP')} ({p.get('title','')})</span>" for p in affected_pol[:3]])
            html += f"<div style='font-size:12px;margin-bottom:6px'><b>Potentially Impacted Policies/WSPs:</b> {pol_refs}</div>"
        html += f"<div style='background:#fffbeb;border-left:3px solid #f59e0b;padding:6px 10px;border-radius:4px;font-size:12px;margin-top:8px'>"
        html += f"<b>Recommended Action:</b> Conduct a targeted review of the identified policies and WSPs against the current rule text. Assign to responsible compliance officer for gap analysis. Document findings."
        html += "</div></div>"
        sources.append(f"{r['id']} – {r['title']}")

    html += "<div style='margin-top:16px;padding-top:10px;border-top:1px solid #e5e7eb;font-size:11px;color:#9ca3af;font-style:italic'>"
    html += "⚠ This output is intended to support compliance review and does not replace human regulatory analysis. All determinations of regulatory applicability should be reviewed by a qualified compliance professional or legal counsel."
    html += "</div></div>"

    # Download content
    download_text = f"REGULATORY UPDATE MEMO\n{'='*50}\nFirm: {firm.get('name')}\nDate: {today_str}\n\n"
    for r in pool:
        download_text += f"\n{r['source']} — {r['title']}\n{'-'*40}\n{r['description']}\n\nRule Requirements:\n{r['rule_requirements']}\n\nRecommended Action: Review linked policies and WSPs for compliance gaps.\n"
...
def page_ai_copilot():
    ...
    <div class='gc-header'>
      <h1>AI COPILOT</h1>
      <p>Ask questions about platform data, policies, testing, or regulatory changes — powered by your firm's compliance records</p>
    </div>""", unsafe_allow_html=True)

    # Firm context pill
    st.markdown(f"""
    <div style='background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:8px 14px;font-size:12px;color:#1e40af;margin-bottom:1rem;display:flex;align-items:center;gap:8px'>
    ...
    st.markdown("<div style='font-size:11px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px'>Suggested prompts</div>", unsafe_allow_html=True)
    cols = st.columns(len(SUGGESTED_PROMPTS))
    for i, (icon, cat, prompt_text) in enumerate(SUGGESTED_PROMPTS):
        with cols[i]:
            if st.button(f"{icon} {prompt_text[:28]}…" if len(prompt_text)>30 else f"{icon} {prompt_text}", key=f"copilot_sug_{i}", use_container_width=True):
                st.session_state.copilot_history.append({"role":"user","content":prompt_text})
                with st.spinner("Analyzing…"):
                    result = _copilot_respond(prompt_text)
    ...
    for i, msg in enumerate(st.session_state.copilot_history):
        if msg["role"] == "user":
            ...
            <div style='display:flex;justify-content:flex-end;margin-bottom:8px'>
              <div style='background:#2563eb;color:#fff;border-radius:12px 12px 2px 12px;padding:9px 14px;max-width:75%;font-size:13px;line-height:1.5'>{msg['content']}</div>
            </div>""", unsafe_allow_html=True)
        else:
            result = msg["content"]
            intent_labels = {"search_platform_records":"🔍 Platform Search","query_platform_data":"📊 Data Query","summarize_testing":"🧪 Testing Summary","generate_report":"📄 Report","regulatory_analysis":"📰 Regulatory Analysis"}
            intent_label = intent_labels.get(result.get("intent",""), "💬 Response")

            st.markdown(f"""
            <div style='background:#fff;border:1px solid #e5e7eb;border-radius:12px 12px 12px 2px;padding:14px 16px;margin-bottom:4px;max-width:90%'>
              <div style='font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.08em;text-transform:uppercase;margin-bottom:10px'>{intent_label}</div>
              {result['html']}
            </div>""", unsafe_allow_html=True)
            ...
            if result.get("sources"):
                ...
                st.markdown(f"<div style='font-size:11px;color:#9ca3af;margin-bottom:2px;padding-left:2px'>📎 Sources: {src_text}</div>", unsafe_allow_html=True)
    ...
    with st.container():
        ...
        with c1:
            user_input = st.text_input(
                "Ask the AI Copilot anything about your compliance program…",
                key="copilot_input",
                placeholder="e.g. Summarize 3120 testing results, show high risk exceptions, which policies need review…",
    ...
    if send and user_input and user_input.strip():
        ...
        with st.spinner("Analyzing…"):
            result = _copilot_respond(user_input.strip())
        st.session_state.copilot_history.append({"role":"assistant","content":result})
        st.rerun()
