...
def page_wsp():
    ...
    def _render_library(docs, tasks_key, task_dict_key, id_prefix, lob_filter, search_filter):
        ...
        for lob in sorted(by_lob.keys()):
            lob_docs = by_lob[lob]
            st.markdown(f"""
            <div style='background:#1e293b;color:#f8fafc;border-radius:8px;
                 padding:.6rem 1rem;margin:1rem 0 .3rem 0;
                 display:flex;justify-content:space-between;align-items:center'>
              <span style='font-size:14px;font-weight:800;letter-spacing:.05em;color:#f8fafc'>📁 {lob.upper()}</span>
              <span style='font-size:12px;color:#94a3b8'>{len(lob_docs)} document(s)</span>
            </div>
            """, unsafe_allow_html=True)
                ...
                <div style='background:#ffffff;border:1px solid #e5e7eb;border-left:4px solid #22c55e;
                  ...
                  <div style='min-width:0;flex:1'>
                    ...
                    <div style='font-size:11px;color:#6b7280;margin-top:3px'>
                      Owner: <b>{owner_name}</b> &nbsp;·&nbsp; Approved by: <b>{approver_name}</b> &nbsp;·&nbsp; {doc['approved_ts'][:10]}
                      {f"&nbsp;·&nbsp; <span style='color:#2563eb'>{task_count} procedure(s)</span>" if task_count else ""}
                      {f"&nbsp;·&nbsp; <span style='color:#7c3aed'>{cl_count} checklist(s)</span>" if cl_count else ""}
                    </div>
                  </div>
    ...
    with tab_pol:
        ...
        with st.expander("➕ New Policy", expanded=False):
    ...
    with tab_wsp:
        ...
        with st.expander("➕ New WSP", expanded=False):
